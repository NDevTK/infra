...
By default this will:
  ...
  * Run the 3pp recipe in experimental mode using `//infra.git/../../.3pp_wd`
    as the default $WORKDIR, using locally *committed* 3pp code as input.
    Non-committed changes are silently ignored. Note that .3pp_wd is OUTSIDE
    of //infra.git and //infra_superproject.git; this is intentional because
    otherwise `git apply` fails to patch.
