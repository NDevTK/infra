...
def print_file(file_name):
  print("printing for file ", file_name)
...
def print_files(dir_name):
    ...
    print("starting for " + dir_name)
    for file in files:
      print(file)
    print("ending for " + dir_name)
...
def nmake_php(out_dir):
  ...
  # res = subprocess.run(["configure.bat", "--help"])
  os.system("configure --help")

  print("configure --help complete")
  
  # res = subprocess.run(["configure.bat", "--disable-cli"])
  res = os.system(f"configure --without-iconv --disable-all --enable-cli --enable-apache2-4handler --with-prefix={out_dir} --with-php-build={out_dir}")

  if res != 0:
    print("configure failed = ", res)
    return sys.exit(res)
  print("configure.bat complete")
  # print_file("./configure.bat")
  # print_lines_with_line_numbers("./configure.js")
  print_files(".")
 # _USE_32BIT_TIME_T
  # replace_line("./win32/build/config.w32.h.in", "_USE_32BIT_TIME_T", "")
  # replace_line("./win32/build/confutils.js", " /wd4996 /D_USE_32BIT_TIME_T=1 ", " /wd4996 ")
  # replace_line_till("./ext/sqlite3/libsqlite/sqlite3.c", "defined(_USE_32BIT_TIME_T)", "#endif")
  # add_to_end_of_file("./win32/build/config.w32.h.in", "#define _USE_64BIT_TIME_T 1")


  
  # print_file("./sapi/cli/php_cli_server.c")
...
def main():
  ...
  # finishing httpd
  print("finished installing httpd")
  out_dir = sys.argv[1]
  print("out_dir=", out_dir)
  print_files(out_dir)
  print_files(f"{out_dir}\\lib")
  print_files(f"{out_dir}\\bin")
  print_files(f"{out_dir}\\include")
