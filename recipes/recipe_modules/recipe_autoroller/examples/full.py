...
def GenTests(api):
  ...
  yield test(
      ...
      api.recipe_autoroller.roll_data('build', repo_spec(self_approve_method='CODE_REVIEW_1_APPROVE')),
  )

  yield test(
      'trivial_code_review_2',
      api.recipe_autoroller.roll_data('build', repo_spec(self_approve_method='CODE_REVIEW_2_APPROVE')),
  )
