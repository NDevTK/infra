...
class CIPDSpec(object):
  ...
  It maps a CIPD (pkg, tag) pair, and allows the following operations:
    * Checking its existance on the CIPD server
  ...
  # (pkg, version) -> instance_id
  _VERSION_CACHE = {}

  def __init__(self, api, pkg, version):
    self._api = api
    self._pkg = str(pkg)
    self._version = str(version)
    assert self._pkg != 'None'
    assert self._version
    assert 'None' not in self._version
    assert self._version != 'implied'
    assert self._version != 'latest'

  def check(self):
    """Returns True if the package is available. This will be true if the
    package is available locally, or if the package is available on the server.
    """
    try:
      self.resolve()
      return True
  ...
  def _resolved_instance_id(self):
    ...
    this returns None."""
    ver_key = (self._pkg, self._version)
  ...
  def _ensure_fetched(self):
    ...
    vers = iid if iid else self._version
    pin = self._api.cipd.pkg_fetch(fetch_path, self._pkg, vers)
    if not iid:
      ver_key = (self._pkg, self._version)
      self._VERSION_CACHE[ver_key] = pin.instance_id
      ...
      self._api.file.move(
        'mv fetched cipd pkg to cache', fetch_path, ret)
    else:
  ...
  def build(self, root, install_mode, version_file):
    ...
    self._VERSION_CACHE[(self._pkg, self._version)] = pin.instance_id
