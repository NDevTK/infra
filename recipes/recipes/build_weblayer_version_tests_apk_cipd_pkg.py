...
def generate_skew_test_config_lines(library, version):
  ...
  lines.extend(
      ' ' * 4 + v for v in  args_tmpl.format(milestone=milestone).splitlines())
  lines.append(' ' * 4 + id_tmpl.format(milestone=milestone))
  lines.extend(
      ' ' * 4 + v for v in SWARMING_ARGS.format(
...
def RunSteps(api):
  ...
  for version in cipd_pkgs_to_create:
    pkg_name = 'version:%s' % version
    pkg_ref = 'm%s' % version[:version.index('.')]
    # Checkout chromium version
    with checkout_chromium_version(api, version):
        # Build weblayer instrumentation tests APK for x86
        pass
