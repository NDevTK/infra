...
PROPERTIES = {
    ...
    'vm2_bundle': Property(
        kind=str, help="The location of the second binary", default=None),
}
...
#  else:
#    api.step('Launching Dual VMs', ["python3", vm_launcher," --binary_path", "binary_loc", "--cpu_count", 4, "--bundle", vm1_bundle, vm2_bundle, "-d"])

def GenTests(api):
 yield api.test('basic')
# yield api.test('One VM',
#   api.properties(vm_count = "1", vm1_bundle="dummy_loc", vm2_bundle="dummy_loc"),
#   api.post_process(StepCommandRE, 'Single VM',
#        ["python3", "~/cr/infra_internal/infra_internal/tools/purple_panda/Python/mac_vm_script.py"," --binary_path", "[CACHE]/purple_panda", "--cpu_count", 4, "--bundle", "dummy_loc"]),
