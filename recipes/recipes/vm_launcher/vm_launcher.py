...
def RunSteps(api, vm_count, vm1_bundle, vm2_bundle):
  ...
  # Download the vm bundle from google cloud storage
  # api.gsutil.download("mac-vm-images", "VM.bundle.zip", api.path['cache'], args=None)
  download_cmd = ["cp", "-r", "gs://mac-vm-images/", api.path['cache']]
  api.gsutil(download_cmd,"Install Bundle")

  bundle_loc = api.path['cache'].join("VM.bundle.zip")
  # Unzip vm bundle
  api.archive.extract("Unzip VM Bundle", bundle_loc, api.path['cache'])
