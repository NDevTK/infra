...
func (p *ManifestParser) parsePool() error {
	// A pool is defined using the `pool` keyword followed by the pool name, a newline,
...
func (p *ManifestParser) parseEdge() error {
	// An edge (action) is defined using the `build` keyword, a space-separated list of output files,
	...
	// - Implicit dependencies (i.e. those not explicitly provided to the command) may be added to the end
...
func (p *ManifestParser) parseRule() error {
	// A rule is defined using the `rule` keyword, the name of the rule, and an indented set of
	// `variable = value` lines. Some variables have special meaning, for example `command` is
...
func (p *ManifestParser) parseDefault() error {
	// A default target is defined using the `default` keyword, followed by a space-separated list of target names.
	// For example, `default foo bar` specifies that `foo` and `baz` will be built by default.
	v, err := p.lexer.Path()
	if err != nil {
		return err
	}
