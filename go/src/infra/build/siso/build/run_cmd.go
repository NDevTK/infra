...
func (b *Builder) runCmd(ctx context.Context, step *Step, allowLocalFallback bool) error {
	...
	// - Allow REAPI if command has platform container-image property.
	...
	allowRemote := len(step.cmd.Platform) > 0 && step.cmd.Platform["container-image"] != ""
	allowREProxy := len(step.cmd.REProxyConfig.Labels) > 0 && experiments.Enabled("use-reproxy", "enable use-reproxy")
	if step.cmd.Pure && b.remoteExec != nil && (allowRemote || allowREProxy) {
		var err error
