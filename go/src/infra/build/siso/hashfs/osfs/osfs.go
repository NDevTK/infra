...
// For FileDigest, if size is non-negative, it will be used.
...
// FileDigest returns file's digest via xattr if possible.
func (fs *OSFS) FileDigest(ctx context.Context, name string, size int64) (digest.Digest, error) {
...
// FileDigest returns file's digest via xattr if possible.
func (fs FileSource) FileDigest(ctx context.Context) (digest.Digest, error) {
	return fs.fs.FileDigest(ctx, fs.Fname, fs.size)
}
