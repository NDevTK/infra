...
// resolveNextDir resolves next dir named `elem` by `next`.
// `next` will return *directory if next is directory.
// `next` will return string if next is symlink.
// it will resolve simple symlnk (i.e. symlink is just base name).
// it returns directory if next is directory.
// it returns resolved path name as string if next is symlink.
func resolveNextDir(ctx context.Context, next func(context.Context, pathElements, string) (*directory, string, bool), pe pathElements, elem, rest string) (*directory, string, bool) {
	i := 0
	for ; i < maxSymlinks; i++ {
		nextDir, target, ok := next(ctx, pe, elem)
		if target != "" {
...
func (d *directory) lookupNextDir(ctx context.Context, pe pathElements, elem string) (*directory, string, bool) {
	v, ok := d.m.Load(elem)
	if ok {
		dent := v.(*entry)
		if dent != nil {
...
// next for store case.
// it will create next dir entry if needed.
func (d *directory) nextDir(ctx context.Context, pe pathElements, elem string) (*directory, string, bool) {
	v, ok := d.m.Load(elem)
	if ok {
		dent := v.(*entry)
