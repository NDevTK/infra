...
func CreateDeviceLeaseRecord(ctx context.Context, tx *sql.Tx, record DeviceLeaseRecord) error {
	stmt, err := tx.PrepareContext(ctx, `
		...
		VALUES
			($1, $2, $3, $4, $5, $6, $7, $8, $9);`)
	if err != nil {
		logging.Errorf(ctx, "CreateDeviceLeaseRecord: failed to prepare insert statement: %s", err)
	}
	defer func() {
		err = stmt.Close()
		if err != nil {
			logging.Debugf(ctx, "CreateDeviceLeaseRecord: failed to close statement: %s", err)
		}
	}()

	result, err := stmt.ExecContext(ctx, record.ID, record.IdempotencyKey,
		record.DeviceID, record.DeviceAddress, record.DeviceType, record.OwnerID,
		record.LeasedTime, record.ExpirationTime, record.LastUpdatedTime)
	if err != nil {
...
func GetDeviceLeaseRecordByID(ctx context.Context, db *sql.DB, recordID string) (DeviceLeaseRecord, error) {
	...
	stmt, err := db.PrepareContext(ctx, `
		...
		WHERE id=$1;`)
	if err != nil {
		logging.Errorf(ctx, "GetDeviceLeaseRecordByID: failed to prepare select statement: %s", err)
		return record, err
	}
	defer func() {
		err = stmt.Close()
		if err != nil {
			logging.Debugf(ctx, "GetDeviceLeaseRecordByID: failed to close statement: %s", err)
		}
	}()
	...
	err = stmt.QueryRowContext(ctx, recordID).Scan(
		&record.ID,
...
func GetDeviceLeaseRecordByIdemKey(ctx context.Context, db *sql.DB, idemKey string) (DeviceLeaseRecord, error) {
	...
	if err != nil {
		logging.Errorf(ctx, "GetDeviceLeaseRecordByIdemKey: failed to prepare select statement: %s", err)
		return record, err
	}
	defer func() {
		err = stmt.Close()
		if err != nil {
			logging.Debugf(ctx, "GetDeviceLeaseRecordByIdemKey: failed to close statement: %s", err)
		}
	}()
	...
	err = stmt.QueryRowContext(ctx, idemKey).Scan(
		&record.ID,
		&record.IdempotencyKey,
		&record.DeviceID,
		&record.DeviceAddress,
