...
func (p *Parser) Parse() (*ExpectationStatement, error) {
	...
	if tok != HASH && tok != LB && tok != IDENT {
		return nil, fmt.Errorf("expected HASH (comment), LB (modifier), or IDENT (start of expectation rule) but found %q", lit)
	}
	...
	tok, lit = p.scanIgnoreWhitespace()
	// Check for optional: HASH comment at the end of a line.
	if tok == HASH {
		stmt.Comment = lit
		ch := p.s.read()
		for ; ch != eof; ch = p.s.read() {
