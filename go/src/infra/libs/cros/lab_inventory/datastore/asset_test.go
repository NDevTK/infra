...
func assertLocationEqual(a, b *fleet.Location) {
	...
	So(a.GetZone(), ShouldEqual, b.GetZone())
}
...
func TestAddAsset(t *testing.T) {
	...
	Convey("Add asset to datastore", t, func() {
		Convey("Add asset to datastore", func() {
			...
			So(res, ShouldHaveLength, 1)
			So(res[0].Err, ShouldBeNil)
			s, err := res[0].StateEntity.ToAssetState()
			So(err, ShouldBeNil)
			So(s.GetState().String(), ShouldEqual, "STATE_ONBOARDING")
		})
		Convey("Add existing asset to datastore", func() {
			...
			s0, err := res[0].StateEntity.ToAssetState()
			So(err, ShouldBeNil)
			So(s0.GetState().String(), ShouldEqual, "STATE_ONBOARDING")
			s1, err := res[1].StateEntity.ToAssetState()
			So(err, ShouldBeNil)
			So(s1.GetState().String(), ShouldEqual, "STATE_ONBOARDING")
		})
