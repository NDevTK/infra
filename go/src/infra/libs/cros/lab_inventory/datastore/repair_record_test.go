...
func TestAddRecord(t *testing.T) {
	...
	ids1 := []string{
		generateRepairRecordID(record1.Hostname, record1.AssetTag),
		generateRepairRecordID(record2.Hostname, record2.AssetTag),
	}

	ids2 := []string{
		generateRepairRecordID(record3.Hostname, record3.AssetTag),
		generateRepairRecordID(record4.Hostname, record4.AssetTag),
	}
...
func TestUpdateRecord(t *testing.T) {
	...
	Convey("Update record in datastore", t, func() {
		Convey("Update existing record to datastore", func() {
			rec1ID := generateRepairRecordID(record1.Hostname, record1.AssetTag)
		...
		Convey("Update non-existent record in datastore", func() {
			rec2ID := generateRepairRecordID(record2.Hostname, record2.AssetTag)
		...
		Convey("Update multiple records to datastore", func() {
			rec3ID := generateRepairRecordID(record3.Hostname, record3.AssetTag)
			rec4ID := generateRepairRecordID(record4.Hostname, record4.AssetTag)
			req := []*inv.DeviceManualRepairRecord{record3, record4}
			res, err := AddDeviceManualRepairRecords(ctx, req)
			So(err, ShouldBeNil)
			So(res, ShouldNotBeNil)
		...
		Convey("Update record without ID to datastore", func() {
			rec5ID := generateRepairRecordID(record5.Hostname, record5.AssetTag)
