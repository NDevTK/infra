...
func TestAddDevices(t *testing.T) {
	...
	Convey("Add device to datastore", t, func() {
		...
		Convey("Add 1 dut and 1 labstation with servo port auto assigned", func() {
			...
			devsToAdd := []*lab.ChromeOSDevice{dut1, labstation}
			...
			// The servo ports are assigned.
			{
				...
				So(int(labConfig.GetDut().GetPeripherals().GetServo().GetServoPort()), ShouldEqual, 9997)
			}
			...
			// attached.
			{
				...
				So(labConfig.GetLabstation().GetServos(), ShouldHaveLength, 4)
				ports := []int{}
				for _, s := range labConfig.GetLabstation().GetServos() {
					ports = append(ports, int(s.GetServoPort()))
				}
				So(ports, ShouldResemble, []int{9999, 9998, 9997, 9996})
			}
		})

		Convey("Add 2 duts and auto assign servo port (labstation will be also created implicity)", func() {
			dut1 := mockDut("dut2-1", "", "labstation20")
			dut2 := mockDut("dut2-2", "", "labstation20")
			dut1.GetDut().GetPeripherals().GetServo().ServoPort = 0
			dut1.GetDut().GetPeripherals().GetServo().ServoSerial = "SN1"
			...
			{
				ports := []int{}
				for _, r := range dsResp.Passed() {
					labConfig := getLabConfigByID(ctx, t, r.Entity.ID)
					ports = append(ports, int(labConfig.GetDut().GetPeripherals().GetServo().GetServoPort()))
				}
				So(ports, ShouldResemble, []int{9999, 9998})
			}

			// There's a labstation saved to datastore and has two servos
			// attached.
			{
				labConfig := getLabConfigByHostname(ctx, t, "labstation20")
				So(labConfig.GetLabstation().GetServos(), ShouldHaveLength, 2)
				ports := []int{}
				for _, s := range labConfig.GetLabstation().GetServos() {
					ports = append(ports, int(s.GetServoPort()))
				}
