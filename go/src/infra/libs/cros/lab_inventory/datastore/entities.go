...
func NewAssetStateEntity(a *fleet.ChopsAsset, state fleet.State, updated time.Time, parent *datastore.Key) (*AssetStateEntity, error) {
	...
	stateBytes, err := proto.Marshal(&fleet.AssetState{
		Id:    a.GetId(),
		State: state,
	})
	if err != nil {
...
// ToAssetState returns an AssetState object
func (e *AssetStateEntity) ToAssetState() (*fleet.AssetState, error) {
	var s fleet.AssetState
	err := proto.Unmarshal(e.State, &s)
	return &fleet.AssetState{
		Id:    e.ID,
		State: s.GetState(),
	}, err
}
