...
func parseHardwareFeatures(components []*api.Component, hf *api.HardwareFeatures) []device.Config_HardwareFeature {
	...
	// HARDWARE_FEATURE_FLASHROM: not used
	// HARDWARE_FEATURE_HOTWORDING: field in topology.Audio hasn't been set
	...
	// HARDWARE_FEATURE_LUCID_SLEEP: a key in powerConfig?
...
func parseCPU(components []*api.Component) device.Config_Architecture {
	for _, c := range components {
		if soc := c.GetSoc(); soc != nil {
