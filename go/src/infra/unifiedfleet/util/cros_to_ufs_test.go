...
import (
	...
	"infra/unifiedfleet/internal/app/frontend/fake"
)
...
func TestParseATLTopology(t *testing.T) {
	Convey("Verify ParseATLTopology", t, func() {
		Convey("happy path", func() {
			resp, err := fake.SheetData("../internal/app/frontend/fake/sheet_data.json")
			So(err, ShouldBeNil)
...
func TestParseOSDhcpdConf(t *testing.T) {
	Convey("Verify ParseOSDhcpdConf", t, func() {
		Convey("happy path", func() {
			...
			So(err, ShouldBeNil)
			topology, _ := ParseATLTopology(resp)
			b, err := fake.GitData("../internal/app/frontend/fake/dhcp_test.conf")
			So(err, ShouldBeNil)
