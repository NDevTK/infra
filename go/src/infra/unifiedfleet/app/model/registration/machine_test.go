...
func TestListMachinesACL(t *testing.T) {
	...
	ctx := gaetesting.TestingContextWithAppID("go-test")
	datastore.GetTestable(ctx).Consistent(true)
	machines := make([]*ufspb.Machine, 20)
	for i := 0; i < 10; i++ {
		chromeOSMachine := mockChromeOSMachine(fmt.Sprintf("chromeos-%d", i), "chromeoslab", "samus", ufspb.Zone_ZONE_CHROMEOS4)
		resp, _ := CreateMachine(ctx, chromeOSMachine)
		machines[i] = resp
	}
	for i := 10; i < 20; i++ {
		chromeOSMachine := mockChromeOSMachine(fmt.Sprintf("chromeos-%d", i), "chromeoslab", "samus", ufspb.Zone_ZONE_CHROMEOS5)
		resp, _ := CreateMachine(ctx, chromeOSMachine)
		machines[i] = resp
	}
