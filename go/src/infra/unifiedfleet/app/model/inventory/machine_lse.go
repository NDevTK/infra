...
func GetMachineLSEIndexedFieldName(input string) (string, error) {
	...
	case util.ServoFilterName:
		field = "servo_id"
	case util.DatacenterFilterName:
		field = "datacenter"
	...
	default:
		return "", status.Errorf(codes.InvalidArgument, "Invalid field name %s - field name for host are machine/machinelseprototype/rpm/vlan/servo/datacenter/rack/switch", input)
	}
	return field, nil
}
