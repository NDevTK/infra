...
func renameLabstation(ctx context.Context, oldName, newName string, lse *ufspb.MachineLSE, machine *ufspb.Machine) error {
	//Get all the duts refrencing the old labstation
	...
	for _, dut := range dutMachinelses {
		...
		dut.GetChromeosMachineLse().GetDeviceLse().GetDut().GetPeripherals().GetServo().ServoHostname = newName
	...
	// Collect all the MLSEs updated
	updatedMLSEs := append(dutMachinelses, lse)
	// Update new labstation history for registration.
	newHc := getHostHistoryClient(lse)
	// Update states
	if err := newHc.stUdt.addLseStateHelper(ctx, lse, machine); err != nil {
		return err
	}
	newHc.LogMachineLSEChanges(nil, lse)
	// Record all the history clients that need to be saved.
	hcs = append(hcs, newHc)
	// Update all the MLSEs affected.
	_, err = inventory.BatchUpdateMachineLSEs(ctx, updatedMLSEs)
	if err != nil {
		return errors.Annotate(err, "Failed to add MachineLSE").Err()
	}
