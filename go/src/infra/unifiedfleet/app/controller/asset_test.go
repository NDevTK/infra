...
func mockAsset(name, aType, model, row, rack, pos, host string, zone ufspb.Zone) *ufspb.Asset {
	return &ufspb.Asset{
		Name:  name,
		Type:  aType,
		Model: model,
...
func TestAssetRegistration(t *testing.T) {
	...
	Convey("Testing AssetRegistration", t, func() {
		Convey("Register machine with existing rack", func() {
			...
			So(err, ShouldBeNil)
			a := mockAsset("C001001", "DUT", "eve", "2", "chromeos6-row2-rack3", "1", "chromeos6-row2-rack3-host1", ufspb.Zone_ZONE_CHROMEOS6)
			_, err = AssetRegistration(ctx, a)
			So(err, ShouldBeNil)
		})
		Convey("Register machine with non-existent rack", func() {
			a := mockAsset("C001002", "DUT", "eve", "2", "chromeos6-row3-rack3", "1", "chromeos6-row2-rack3-host1", ufspb.Zone_ZONE_CHROMEOS6)
			_, err := AssetRegistration(ctx, a)
		...
		Convey("Register machine with invalid name", func() {
			...
			_, err := registration.CreateRack(ctx, r)
			So(err, ShouldBeNil)
			a := mockAsset("", "DUT", "eve", "4", "chromeos6-row4-rack3", "1", "chromeos6-row4-rack3-host1", ufspb.Zone_ZONE_CHROMEOS6)
			_, err = AssetRegistration(ctx, a)
			So(err, ShouldNotBeNil)
		})
	})
