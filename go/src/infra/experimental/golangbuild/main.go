...
func runSubrepoTests(ctx context.Context, goroot string) (err error) {
	...
	type ann struct {
		...
		Env   []string // Extra environment to append.
	}
	...
	if err := runGo(ann{Name: "download subrepos and deps", Infra: true, Env: []string{"GOPROXY=direct"}}, "list", "-mod=mod", "-deps", "-test", "golang.org/x/mod/...", "golang.org/x/term/..."); err != nil {
		return err
	}
