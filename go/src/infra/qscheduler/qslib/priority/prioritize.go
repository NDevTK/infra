...
// TODO: this function doesn't actually belong in this module, it should go
// with the account-updating code.
func countJobsPerPriorityPerAccount(running []*task.Running, balances *map[account.ID]account.Balance) map[account.ID]*account.IntVector {
	jobsPerPriorityPerAccount := make(map[account.ID]*account.IntVector)
	for key := range *balances {
		jobsPerPriorityPerAccount[key] = &account.IntVector{}
	}

	for _, running := range running {
		_, ok := jobsPerPriorityPerAccount[running.Request.AccountID]
		if ok {
			priority := running.Priority
			if priority >= 0 && priority < account.FreeBucket {
				jobsPerPriorityPerAccount[running.Request.AccountID][priority]++
			}
		}
	}

	return jobsPerPriorityPerAccount
}

func demoteJobsBeyondFanout(prioritizedRequests List, state *types.State, config *types.Config) {
	jobsPerAccount := make(map[account.ID]int)
...
func PrioritizeRequests(state *types.State, config *types.Config) List {
	...
	less := func(i, j int) bool {
		...
		return a.Request.EnqueueTime.Before(b.Request.EnqueueTime)
	}
...
// findPriority takes an already sorted prioritizedRequests slice, and
...
func (pR List) ForPriority(priority int) List {
	start := len(pR)
	end := len(pR)
	for i := 0; i < len(pR); i++ {
		p := pR[i].Priority
