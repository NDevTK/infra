...
// OrderedRequests represents a priority-sorted list of requests.
...
func PrioritizeRequests(state *types.State, config *types.Config) OrderedRequests {
	...
	// balance.
	list := OrderedRequests(make([]Request, 0, len(state.Requests)))
...
func requestsListLess(l OrderedRequests, i, j int) bool {
	a := l[i]
	b := l[j]
