...
// NewState returns a new initialized State instance.
func NewState() *State {
	return &State{
		Config:       &Config{},
...
// ReapingWorker represents a worker that is idle and wants to reap a task.
...
func (state *State) Reap(s Scheduler, workers []*ReapingWorker, t time.Time) []*Assignment {
	...
	// Step 2: Determine which reaping workers should be marked as idle (because
	// they don't have anything already enqueued). Mark these as idle.

	// Labels for idle workers, by id.
	idleWorkerLabels := make(map[string]task.LabelSet)
	for _, w := range workers {
		if q := state.WorkerQueues[w.ID]; q == nil {
			idleWorkerLabels[w.ID] = w.ProvisionableLabels
		}
	}
	for id, labels := range idleWorkerLabels {
		s.MarkIdle(id, labels)
	}
...
// GetCancellations will continue to return their previous results).
