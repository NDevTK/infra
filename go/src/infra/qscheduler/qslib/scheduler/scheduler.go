...
func matchIdleBotsWithLabels(state *types.State, jobsAtP priority.List, output chan types.Mutater) {
	for i, request := range jobsAtP {
		...
		for _, worker := range state.Workers {
			if worker.IsIdle() && task.LabelSet(worker.Labels).Equals(request.Request.Labels) {
...
// priority.
func reprioritizeRunningTasks(state *types.State, config *types.Config, priority int32, output chan types.Mutater) {
...
func preemptRunningTasks(state *types.State, jobsAtP []priority.Request, priority int32, output chan types.Mutater) {
	candidates := make([]*types.Worker, 0, len(state.Workers))
