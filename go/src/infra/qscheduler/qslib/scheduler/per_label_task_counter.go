...
// tasksPerLabelKeyVal tracks how many concurrent tasks are running for unique
// label keyval combinations.
type tasksPerLabelKeyVal map[string]int32

// perLabelTaskCounter counts how many concurrent tasks per unique label keyval
// are running for each account that has per-label task limits set for some
// labels.
...
// newPerLabelTaskCounter initializes a perLabelTaskCounter.
