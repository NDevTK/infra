...
func TestReprioritize(t *testing.T) {
	...
	config := types.Config{
		AccountConfigs: map[string]*account.Config{
			"a1": &account.Config{ChargeRate: vector.Ref(vector.V{1.5, 1.5})},
		},
	}
	state := types.State{
		Balances: map[string]*vector.Vector{
			"a1": vector.Ref(vector.V{2 * account.DemoteThreshold, 2 * account.PromoteThreshold, 0}),
		},
		Workers: map[string]*types.Worker{
			"w1": &types.Worker{Id: "w1", RunningTask: &task.Run{
				Cost:     vector.Ref(vector.V{1}),
				Priority: 0,
...
func TestPreempt(t *testing.T) {
	...
	state := types.State{
		Balances: map[string]*vector.Vector{
			"a1": vector.Ref(vector.V{}),
			"a2": vector.Ref(vector.V{1}),
		},
		RequestQueue: map[string]*task.Request{
		...
		Workers: map[string]*types.Worker{
			"w1": &types.Worker{
				Id: "w1",
				RunningTask: &task.Run{
					Cost:     vector.Ref(vector.V{.5, .5, .5}),
					Priority: 1,
					Request:  &task.Request{Id: "t2", AccountId: "a1"},
				},
			},
