...
func TestStopRequest(t *testing.T) {
	...
	reqID := "request1"
	wID := "worker1"
	Convey("Given a state with one request and one idle workers", t, func() {
		state := NewState(tm0)
		state.addRequest(reqID, NewRequest("", nil, tm1), tm1)
		state.markIdle(wID, []string{}, tm1)
		stateBefore := state.Clone()
		Convey("when StopRequest with forward time is called for that request", func() {
		...
		Convey("when StopRequest with backward time is called for that request", func() {
