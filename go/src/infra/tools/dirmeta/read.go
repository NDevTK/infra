...
// r.Mapping.
//
// If full is true, the r.Mapping has an entry for each directory in
// the tree ( even if that directory does not define any metadata), and the
// entry includes inherited metadata.
// If full is false, then the r.Mapping represents data from the files as is.
// In particular, it is neither reduced, nor represents inherited metadata.
func (r *MappingReader) ReadAll(full bool) error {
	...
	return filepath.Walk(r.Root, func(dir string, info os.FileInfo, err error) error {
		...
		if full {
			...
			// expanded parent.
			meta := cloneMD(r.Dirs[path.Dir(key)])
