...
func combineIsolates(ctx context.Context, arc *archiver.Archiver, command []string, isoHashes ...isolated.HexDigest) (isolated.HexDigest, error) {
	iso := isolated.New()
	iso.Command = command
...
func mkArchiver(ctx context.Context, isolatedFlags isolatedclient.Flags, authClient *http.Client) *archiver.Archiver {
	...
	return archiver.New(isoClient, w, verbose)
}
