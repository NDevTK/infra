...
type kitchenProperties struct {
	...
	EmulateCloud bool `json:"emulate_cloud"`
...
func (c *cookRun) prepareProperties(env environ.Env) (map[string]interface{}, *kitchenProperties, error) {
	...
	kitchenProps := &kitchenProperties{
		GitAuth:      true,
		DevShell:     true,
		EmulateCloud: false, // TODO(vadimsh): Make it 'true' if it works reliably.
		DockerAuth:   true,
		FirebaseAuth: false,
	}
	...
	// EmulateCloud supersedes DevShell.
	if kitchenProps.EmulateCloud {
...
func (c *cookRun) setupAuth(ctx context.Context) error {
	...
	systemAuth := &authctx.Context{
		...
		EnableCloudEmulation: c.kitchenProps.EmulateCloud,
