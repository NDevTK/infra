...
func pep425TagSelector(tags []*vpython.PEP425Tag) *vpython.PEP425Tag {
	...
	isBetter := func(t *vpython.PEP425Tag) bool {
		...
		case best.Abi != "cp38" && t.Abi == "cp38":
			...
			// the lack of proper compatible ABI detection. Without this we'd
			// just blindly select the first ABI tag our interpreter supports,
			// and then if there isn't a package built for that specific ABI,
			// fail.
			//
			// Better would be to check which packages are actually available
			// and select the ABI tag based on that. Until then, we work around
			// it by just preferring cp38, as that's currently the only ABI
			// that dockerbuild builds against for Python 3.
			return true
