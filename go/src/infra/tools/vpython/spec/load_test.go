...
func TestLoadForScript(t *testing.T) {
	...
	goodSpec := &env.Spec{
		...
		Wheel: []*env.Spec_Package{
			&env.Spec_Package{Path: "foo/bar", Version: "1"},
			&env.Spec_Package{Path: "baz/qux", Version: "2"},
		},
	}
	goodSpecData := proto.MarshalTextString(goodSpec)
	badSpecData := "foo: bar"

	Convey(`Test LoadForScript`, t, testfs.MustWithTempDir("TestLoadForScript", func(tdir string) {
		...
		Convey(`Layout: module with a bad spec file`, func() {
			...
			So(err, ShouldErrLike, "failed to unmarshal env.Spec")
		})
