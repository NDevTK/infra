		...
		for _, randomized := range []bool{
			...
			Convey(title, func() {
				for _, tc := range testCases {
					// Generate our tag string. We will use "tc.tags" to seed this instead
					// of our actual "tags" because this must be consistent across Convey
					// passes.
					...
					t.Logf("Using tags: %v", tags)

					Convey(fmt.Sprintf(`On OS %q, generates template for [%s]`, tc.goOS, strings.Join(tagsStr, ", ")), func() {
