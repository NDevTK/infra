...
import (
	...
	"go.chromium.org/luci/common/system/testfs"
	...
	Convey(fmt.Sprintf(`Testing %q`, tc), t, testfs.MustWithTempDir(t, "vpython", func(td string) {
		...
		case "test_signals":
			...
			if runtime.GOOS != "windows" {
				tc.runTestSignals(t, c, td)
			}
		case "test_exit_code":
			tc.runCommon(t, c, td, 42)
		default:
			tc.runCommon(t, c, td, 0)
		}
	}))
}

func (tc *testCase) runCommon(t *testing.T, c context.Cntext, td string, exitCode int) {
...
func (tc *testCase) runTestSignals(t *testing.T, c context.Cntext, td string) {
