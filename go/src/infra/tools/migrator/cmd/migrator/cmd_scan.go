...
func (r *cmdScanImpl) execute(ctx context.Context) error {
	return withPlugin(ctx, r.projectDir, func(factory migrator.InstantiateAPI) error {
		...
		projectsCfg, err := client.GetConfig(ctx, config.ServiceSet("luci-config"), "projects.cfg", false)

		projectPB := &configpb.ProjectsCfg{}
		if err := lucipb.UnmarshalTextML(projectsCfg.Content, projectPB); err != nil {
			panic(errors.Annotate(err, "parsing luci-config projects.cfg").Err())
		}

		var allReportsMu sync.Mutex
			...
			for _, projPB := range projectPB.Projects {
				...
				ch <- func() (err error) {
					r.perProjectContext(ctx, projPB, func(ctx context.Context) bool {
						defer func() {
							reports := plugsupport.DumpReports(ctx)

							allReportsMu.Lock()
							numReports := allReports.Update(reports)
							allReportsMu.Unlock()
