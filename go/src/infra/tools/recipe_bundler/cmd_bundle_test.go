...
func TestRepoInputParsing(t *testing.T) {
	...
	goodCases := []struct {
		...
		{`basic repo`, map[string]string{"foo.bar/repo": ""}, map[string]fetchSpec{
			"foo.bar/repo": {"FETCH_HEAD", "HEAD"},
		}},
		{`basic repo with ref`, map[string]string{"foo.bar/repo": "FETCH_HEAD,refs/thingy"}, map[string]fetchSpec{
			"foo.bar/repo": {"FETCH_HEAD", "refs/thingy"},
		}},
		{`basic repo with rev`, map[string]string{"foo.bar/repo": strings.Repeat("deadbeef", 5)}, map[string]fetchSpec{
			"foo.bar/repo": {strings.Repeat("deadbeef", 5), "HEAD"},
		}},
		{`basic repo with rev+ref`, map[string]string{
			"foo.bar/repo": strings.Repeat("deadbeef", 5) + ",refs/foobar",
		}, map[string]fetchSpec{
			"foo.bar/repo": {strings.Repeat("deadbeef", 5), "refs/foobar"},
		}},
		{`basic repo with FETCH_HEAD+ref`, map[string]string{
			"foo.bar/repo": "FETCH_HEAD,refs/foobar",
		}, map[string]fetchSpec{
			"foo.bar/repo": {"FETCH_HEAD", "refs/foobar"},
		}},
		{`basic repo with FETCH_HEAD+HEAD (silly but correct)`, map[string]string{
			"foo.bar/repo": "FETCH_HEAD,HEAD",
		}, map[string]fetchSpec{
			"foo.bar/repo": {"FETCH_HEAD", "HEAD"},
		}},
