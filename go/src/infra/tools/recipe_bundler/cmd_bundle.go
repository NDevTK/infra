...
func bundleCmd(authOpts auth.Options) *subcommands.Command {
	...
	A git revision may be provided with an =, e.g. 'host.name/to/repo=dead...beef'.
	...
	'host.name/to/repo=dead...beef,refs/other/thing'.`)
...
func (c *cmdBundle) parseFlags() (err error) {
	...
	c.authOpts, err = c.authFlags.Options()
	return err
}

func (c *cmdBundle) getCipdClient(ctx context.Context) (cipd.Client, error) {
	var err error
	if c.cipdClient == nil {
		...
		c.cipdClient, err = cipd.NewClient(cipdOpts)
	}
	return c.cipdClient, err
}
