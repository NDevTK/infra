...
func (r *goRun) generateTestResults(ctx context.Context, data []byte) ([]*sinkpb.TestResult, error) {
	ordered, byID := goTestJsonToTestRecords(ctx, data, r.PrintTestOutputToStdout)
...
// stdoutOutput controls whether a copy of test output is printed to stdout.
...
func goTestJsonToTestRecords(ctx context.Context, data []byte, stdoutOutput bool) ([]*TestRecord, map[string]*TestRecord) {
	...
	for lines.Scan() {
		...
		if stdoutOutput {
			os.Stdout.WriteString(tEvt.Output)
		}
	}
	return ordered, byID
}
