...
// Property represents a property value.
...
func (r *GTestResults) convertTestResult(ctx context.Context, buf *bytes.Buffer, testID, name string, result *GTestRunResult) (*sinkpb.TestResult, error) {
	...
	// Parse properties to test tags.
	// tags format should be key1=value1;key2=value2
	if len(result.Properties) > 0 {
		for pName, pValue := range result.Properties {
			p := new(Property)
			if pName == "gtest_tag" && json.Unmarshal(pValue, p) == nil {
				tagPairs := strings.Split(p.Value, ";")
				for _, tagPair := range tagPairs {
					tag := strings.Split(tagPair, "=")
					if len(tag) == 2 {
						tr.Tags = append(tr.Tags, pbutil.StringPair(tag[0], tag[1]))
					}
				}
			}
		}
