...
func TestMetrics(t *testing.T) {
	...
	container.Names = []string{"/container_name"}
	container.State = "running"
	containerInfoBase := dockerTypes.ContainerJSONBase{}
	containerInfo := dockerTypes.ContainerJSON{}
	containerInfo.ContainerJSONBase = &containerInfoBase

	containerState := dockerTypes.ContainerState{StartedAt: nowMinus10s.Format(time.RFC3339Nano)}
	containerInfoBase.State = &containerState

	containerConfig := dockerContainerTypes.Config{}
	containerConfig.Hostname = "hostname123"
	containerInfo.Config = &containerConfig
	containerStatsJSON := dockerTypes.ContainerStats{}

	bodyJSON := "{\"name\": \"container1\", \"memory_stats\": {\"usage\": 1111, \"limit\": 9999}, \"networks\": {\"eth0\": {\"rx_bytes\": 987, \"tx_bytes\": 123}}}"
