...
func (p *pinpointClientFactory) NewClient(ctx context.Context, providerURL string) (pinpoint.PinpointClient, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	tok, err := getUserToken(ctx, p.TCache, providerURL)
	if err != nil {
		return nil, errors.Annotate(err, "failed to get user token").Err()
	}

	log.Printf("Connecting to %s", p.Endpoint)
	conn, err := grpc.Dial(
		p.Endpoint,
		grpc.WithTransportCredentials(p.TLSCredentials),
		grpc.WithPerRPCCredentials(newJWTCredentials([]byte(tok))),
	)
	if err != nil {
		return nil, err
	}
...
func getUserToken(ctx context.Context, tCache *tokenCache, providerURL string) (string, error) {
	p, err := oidc.NewProvider(ctx, providerURL)
	if err != nil {
		return "", errors.Annotate(err, "failed to reach provider %q", providerURL).Err()
	}
	...
	c := oauth2.Config{
		...
		ClientID:     "62121018386-kbva23qhdraiklj3ksdc8lams93q4gk6.apps.googleusercontent.com",
		ClientSecret: "SUeCohQkddFi6gs737yE1k_y",
		Endpoint:     p.Endpoint(),
		Scopes:       authScopes,
	...
	if tCache != nil {
		t, _, err := tCache.GetVerifiedOrRefreshedToken(ctx, v, &c)
		if err == nil {
			return string(t), nil
		}
		logging.Get(ctx).Errorf("failed to get a cached token: %q", err)
	}

	// Create a random 32 byte sequence, then base64 encode it.
	sb := make([]byte, 32)
	if _, err := rand.Read(sb); err != nil {
		return "", errors.Annotate(err, "failed reading random bytes").Err()
	}
	...
	if _, err := fmt.Scan(&code); err != nil {
		return "", errors.Annotate(err, "failed reading code").Err()
	}

	// Here we'll get the code and exchange it for an oauth2 token.
	t, err := c.Exchange(ctx, code)
	if err != nil {
		return "", errors.Annotate(err, "failed exchanging code for an oauth2 token").Err()
	}

	// Let's pull out the ID token from the response.
	rt, ok := t.Extra("id_token").(string)
	if !ok {
		return "", errors.Reason("failed to find an id_token in the response").Err()
	}

	_, err = v.Verify(ctx, rt)
	if err != nil {
		return "", errors.Annotate(err, "failed verifying retrieved id_token").Err()
	}

	// Before continuing, we should update the cache, and log errors.
	if tCache != nil {
		if _, _, err := tCache.UpdateToken(ctx, t, v); err != nil {
			logging.Get(ctx).Errorf("failed updating token cache: %q", err)
		}
	}
	return rt, nil
}
