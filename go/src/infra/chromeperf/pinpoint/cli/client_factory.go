...
func (p *pinpointClientFactory) NewClient(ctx context.Context) (pinpoint.PinpointClient, error) {
	...
	v, o, err := newTokenVerifierAndConfig(ctx, p.TCache)
	if err != nil {
...
func newTokenVerifierAndConfig(
	ctx context.Context,
	tCache *tokenCache) (*oidc.IDTokenVerifier, *oauth2.Config, error) {
	...
	if _, err := fmt.Scan(&code); err != nil {
		return nil, nil, errors.Annotate(err, "failed obtaining code from stdin").Err()
	}

	// Here we'll get the code and exchange it for an oauth2 token.
	t, err := c.Exchange(ctx, code)
	if err != nil {
		return nil, nil, errors.Annotate(err, "failed exchanging code for an oauth2 token").Err()
	}
