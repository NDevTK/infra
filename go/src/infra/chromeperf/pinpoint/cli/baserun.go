...
func (r *baseCommandRun) RegisterDefaultFlags(p Param) {
	...
	`))
	r.Flags.StringVar(&r.oidcProvider, "oidc-issuer", p.OIDCProviderURL, text.Doc(`
		OpenID Connect token provider.
	`))
}
...
func getFactorySettings(ctx context.Context, serviceDomain string) (
	*tokenCache, credentials.TransportCredentials, error) {
	...
	if err != nil {
		return nil, nil, errors.Annotate(err, "failed using system cert pool").Err()
	}
