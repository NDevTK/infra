...
func writeToTempFile(fileName string, data []byte) (string, error) {
	...
	if _, err := f.Write(data); err != nil {
		if cerr := f.Close(); cerr != nil {
			return "", errors.Annotate(err, "failed writing data").
				InternalReason("failed closing file %q", cerr).Err()
		}
...
func newTokenCache(ctx context.Context, dir string) (*tokenCache, error) {
	...
	lf, err := os.OpenFile(filepath.Join(dir, ".cache-lock"), os.O_CREATE|os.O_RDONLY, 0600)
	if err != nil {
		return nil, fmt.Errorf("Failed creating the lock file: %s", err)
	}
	defer func() {
...
func (t *tokenCache) GetVerifiedOrRefreshedToken(
	ctx context.Context, verifier *oidc.IDTokenVerifier, oauth2Config *oauth2.Config) (
	[]byte, *oidc.IDToken, error) {
	...
	if cachedToken.ParsedIDToken.Expiry.Before(time.Now().Add(time.Minute * -5)) {
		...
		src := oauth2Config.TokenSource(ctx, &cachedToken.AuthToken)
		newToken, err := src.Token()
		if err != nil {
			logging.Get(ctx).Debugf("error: %q", err)
		...
		if newToken.AccessToken != cachedToken.AuthToken.AccessToken {
			...
			cit, pit, err := t.UpdateToken(ctx, newToken, verifier)
			if err != nil {
				logging.Get(ctx).Debugf("error: %q", err)
...
func (t *tokenCache) UpdateToken(ctx context.Context, token *oauth2.Token, verifier *oidc.IDTokenVerifier) ([]byte, *oidc.IDToken, error) {
	...
	logging.Get(ctx).Debugf("id_token: %q", idToken)
	if !ok {
		return nil, nil, errors.Reason("token does not include an id_token").Err()
	}

	// Parse the token we got.
	pit, err := verifier.Verify(ctx, idToken)
	if err != nil {
		logging.Get(ctx).Debugf("verification failed: %s", err)
