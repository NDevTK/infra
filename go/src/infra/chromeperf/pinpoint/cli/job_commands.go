	...
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			gj.Flags.StringVar(&gj.name, "name", "", text.Doc(`
				...
				Example: "--name=jobs/legacy-XXXXXXXXXXXXXX"
			`))
			return gj
		},
	}
...
func (gj *getJob) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	if gj.name == "" {
		return gj.done(a, errors.New("must set --name"))
	}
	...
	req := &pinpoint.GetJobRequest{Name: gj.name}
	resp, err := c.GetJob(ctx, req)
	if err != nil {
		return gj.done(a, fmt.Errorf("failed during GetJobs: %v", err))
	}
	out := prototext.MarshalOptions{Multiline: true}.Format(resp)
	fmt.Println(out)
...
func (wj *waitJob) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	if wj.name == "" {
		return wj.done(a, errors.New("must set --name"))
	}
	...
	req := &pinpoint.GetJobRequest{Name: wj.name}
	...
	for {
		resp, err := c.GetJob(ctx, req)
		if err != nil {
			return wj.done(a, fmt.Errorf("failed during GetJobs: %v", err))
		}
		...
		select {
		case <-ctx.Done():
			return wj.done(a, fmt.Errorf("polling for wait-job cancelled (%v)", ctx.Err()))
		case <-poll.C:
