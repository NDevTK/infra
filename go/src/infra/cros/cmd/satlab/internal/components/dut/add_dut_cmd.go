...
import (
	...
	"infra/cros/cmd/satlab/internal/components/dut/internal/shivas"
...
func (c *addDUT) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	c.qualifiedHostname = site.MaybePrepend("satlab", dockerHostBoxIdentifier, c.hostname)
	c.qualifiedRack = site.MaybePrepend("satlab", dockerHostBoxIdentifier, c.rack)
	if c.servo == "" {
		...
		c.qualifiedServo = site.MaybePrepend("satlab", dockerHostBoxIdentifier, c.servo)
	}
