...
func AddDUT(serviceAccountJSON string, satlabPrefix string, p *parse.CommandParseResult) error {
	...
	if err := addAssetIfApplicable(serviceAccountJSON, satlabPrefix, p, rack, zone, model, board); err != nil {
		return errors.Annotate(err, "add dut").Err()
	}
...
func addAssetIfApplicable(serviceAccountJSON string, satlabPrefix string, p *parse.CommandParseResult, rack string, zone string, model string, board string) error {
