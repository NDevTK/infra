...
func (m myjobRunBase) runBBCmd(ctx context.Context, subcommand string, args ...string) (stdout, stderr string, err error) {
	return m.RunCmd(ctx, "bb", prependString("add", args)...)
}

// BBAdd runs a `bb add` command, and prints stdout to the user.
func (m myjobRunBase) BBAdd(ctx context.Context, args ...string) error {
