...
func populateBuildMetadata(
	...
	if chameleonTypes := getTagValues(botDims, "label-chameleon_type"); chameleonTypes != nil {
		chameleonInfo.ChameleonType = getChameleonTypes(chameleonTypes)
	}
	if chameleonConnectionTypes := getTagValues(botDims, "label-chameleon_connection_types"); chameleonConnectionTypes != nil {
		chameleonInfo.ChameleonConnectionTypes = getChameleonConnectionTypes(chameleonConnectionTypes)
	}
...
func getChameleonTypes(chameleonTypeStrings []string) []artifactpb.BuildMetadata_ChameleonType {
	...
	for _, str := range chameleonTypeStrings {
		if index, exists := artifactpb.BuildMetadata_ChameleonType_value[str]; exists {
			chameleonType := artifactpb.BuildMetadata_ChameleonType(index)
...
// based on given string set
func getChameleonConnectionTypes(chameleonConnectionStrings []string) []artifactpb.BuildMetadata_ChameleonConnectionType {
	...
	for _, str := range chameleonConnectionStrings {
		if index, exists := artifactpb.BuildMetadata_ChameleonConnectionType_value[str]; exists {
			chameleonConnectionType := artifactpb.BuildMetadata_ChameleonConnectionType(index)
