...
import (
	...
	infrassh "infra/cros/cmd/common-tls/internal/ssh"
)
...
func runCmd(c *infrassh.Client, cmd string) error {
	s, err := c.NewSession()
	if err != nil {
		return err
	}
...
func runCmdOutput(c *infrassh.Client, cmd string) (string, error) {
	s, err := c.NewSession()
	if err != nil {
		return "", err
	}
...
func stopSystemDaemons(c *infrassh.Client) {
...
func getBuilderPath(c *infrassh.Client) (string, error) {
...
func getRootDev(c *infrassh.Client) (rootDev, error) {
	var r rootDev
...
func clearInactiveDLCVerifiedMarks(c *infrassh.Client, r rootDev) error {
...
func installKernel(c *infrassh.Client, imagePath, kernPartition string) error {
...
func installRoot(c *infrassh.Client, imagePath, rootPartition string) error {
	// TODO(crbug.com/1077056): Use CacheForDut from TLW server for images that
	// need to be fetched. (e.g. kernel, root, stateful, DLCs, etc)
	pathPrefix, err := getGsCacheURL(imagePath)
	if err != nil {
...
func installStateful(c *infrassh.Client, imagePath string) error {
	// TODO(crbug.com/1077056): Use CacheForDut from TLW server for images that
	// need to be fetched. (e.g. kernel, root, stateful, DLCs, etc)
	pathPrefix, err := getGsCacheURL(imagePath)
	if err != nil {
...
func revertStatefulInstall(c *infrassh.Client) {
	const (
		varNew      = "var_new"
...
func installPartitions(c *infrassh.Client, imagePath string, partitions partitionInfo) []error {
...
func postInstall(c *infrassh.Client, partitions partitionInfo) error {
	return runCmd(c, strings.Join([]string{
		"tmpmnt=$(mktemp -d)",
...
func revertPostInstall(c *infrassh.Client, partitions partitionInfo) {
...
func clearTPM(c *infrassh.Client) error {
	return runCmd(c, "crossystem clear_tpm_owner_request=1")
}

func runLabMachineAutoReboot(c *infrassh.Client) {
	const (
...
func provisionOS(c *infrassh.Client, op *longrunning.Operation, imagePath string, r rootDev) error {
	stopSystemDaemons(c)
...
func verifyOSProvision(c *infrassh.Client, op *longrunning.Operation, expectedBuilderPath string) error {
...
func pathExists(c *infrassh.Client, path string) (bool, error) {
...
func isDLCVerified(c *infrassh.Client, spec *tls.ProvisionRequest_DLCSpec, slot dlcSlot) (bool, error) {
	dlcID := spec.GetId()
...
func installDLC(c *infrassh.Client, spec *tls.ProvisionRequest_DLCSpec, imagePath, dlcOutputDir string, slot dlcSlot) error {
...
func provisionDLCs(c *infrassh.Client, op *longrunning.Operation, imagePath string, r rootDev, specs []*tls.ProvisionRequest_DLCSpec) error {
	if len(specs) == 0 {
		return nil
	}
