...
// specified program.
func getProjects(projectPatterns []string, allProjects []string) ([]string, error) {
...
func (b *projectBuildspec) CreateBuildspecs(gsClient gs.Client, gerritClient *gerrit.Client) error {
	...
	if err != nil {
		return errors.Annotate(err, "failed to fetch all projects").Err()
	}
	projects, err := getProjects(b.projects, allProjects)
	if err != nil {
