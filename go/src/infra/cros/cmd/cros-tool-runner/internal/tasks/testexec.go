...
func (c *runTestCmd) innerRun(ctx context.Context, a subcommands.Application, args []string, env subcommands.Env) (*api.CrosToolRunnerTestResponse, error) {
	...
	if device == nil {
		return nil, errors.New("requests does not specify primary device")
	}
	...
	return result, errors.Annotate(err, "failed to run tests").Err()
}
...
func readTestRequest(p string) (*api.CrosToolRunnerTestRequest, error) {
	...
	r, err := os.Open(p)
	if err != nil {
		return nil, errors.Annotate(err, "read test request %q", p).Err()
	}
	err = jsonpb.Unmarshal(r, in)
	return in, errors.Annotate(err, "read test request %q", p).Err()
}
...
func saveTestOutput(out *api.CrosToolRunnerTestResponse, outputPath string) error {
	if outputPath != "" && out != nil {
		...
		if err := os.MkdirAll(dir, 0777); err != nil {
			return errors.Annotate(err, "failed to create directory while saving output").Err()
		}
		f, err := os.Create(outputPath)
		if err != nil {
			return errors.Annotate(err, "failed to create file while saving output").Err()
		}
		defer f.Close()
		marshaler := jsonpb.Marshaler{}
		if err := marshaler.Marshal(f, out); err != nil {
			return errors.Annotate(err, "failed to marshal result while saving output").Err()
		}
		if err := f.Close(); err != nil {
			return errors.Annotate(err, "failed to close file while saving output").Err()
		}
	}
	return nil
}
