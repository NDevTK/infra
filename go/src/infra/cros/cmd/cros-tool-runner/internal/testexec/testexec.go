...
func Run(ctx context.Context, req *api.CrosToolRunnerTestRequest, crosTestContainer *build_api.ContainerImageInfo) (res *api.CrosToolRunnerTestResponse, err error) {
	...
	if err != nil {
		return nil, errors.Annotate(err, "failed to resolve artifact directory %v", req.ArtifactDir).Err()
	}
	...
	if err := os.MkdirAll(crosTestDir, 0755); err != nil {
		return nil, errors.Annotate(err, "failed to create directory %s", crosTestDir).Err()
	}
	if err := os.MkdirAll(resultDir, 0755); err != nil {
		return nil, errors.Annotate(err, "failed to create directory %s", resultDir).Err()
	}

	// create input file.
	inputFileName := path.Join(crosTestDir, "request.json")
	runScriptName := path.Join(crosTestDir, "run")

	if req.PrimaryDut == nil {
		return nil, errors.Reason("run test: primary DUT is not specified").Err()
	}
	primaryDut := req.PrimaryDut.GetDut()
	...
	if err := writeTestInput(inputFileName, testReq); err != nil {
		return nil, errors.Annotate(err, "failed to create input file %s", inputFileName).Err()
	}
	if err = services.RunTestCLI(ctx, crosTestContainer, networkName, inputFileName, crosTestDir, resultDir, runScriptName); err != nil {
		return nil, errors.Annotate(err, "run test").Err()
	}
