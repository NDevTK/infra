...
func RunPreProcessCLI(ctx context.Context, image *build_api.ContainerImageInfo, networkName, preProcessDir string, tokenFile string) error {
	...
	// The files or directories used by pre-process container is set up this way.
	// File or Directory inside the container   Source
	// ++++++++++++++++++++++++++++++++++++++   +++++++++++++++++++++++++++++++++++++++++++
	// /tmp/test/pre-process                    Mount /tmp/test/pre-process during run
	// /tmp/test/pre-process/request.json       Generated before execute pre-process
	// /tmp/test/pre-process/result.json        Generated by pre-process
	// /usr/bin/pre-process                     Included in container image
	d := &docker.Docker{
		...
		Volumes: []string{
			fmt.Sprintf("%s:%s", preProcessDir, filepath.Join(CrosTestFinderRootDirInsideDocker, "pre-process")),
		},
		Detach:  false,
		Network: networkName,
	}
