...
type filterAlreadyUploadedRequestBody struct {
...
// crashRetrieveUploadInforation retrieves a unique upload url and key for the given symbol
// file.
func crashRetrieveUploadInforation(uploadInfo *crashUploadInformation, crash crashConnectionInfo) (int, error) {
...
func crashUploadSymbol(symbolPath, uploadUrl string, crash crashConnectionInfo) (int, error) {
...
func crashSubmitSymbolUpload(uploadKey string, task taskConfig, crash crashConnectionInfo) (int, error) {
...
func upload(task *taskConfig, crash crashConnectionInfo) bool {
	...
	statusCode, err := crashRetrieveUploadInforation(&uploadInfo, crash)
	...
	uploadLogger.Printf("%s: %d\n", crash.url, statusCode)

	statusCode, err = crashUploadSymbol(task.symbolPath, uploadInfo.UploadUrl, crash)
	...
	statusCode, err = crashSubmitSymbolUpload(uploadInfo.UploadKey, *task, crash)
...
func filterTasksAlreadyUploaded(tasks []taskConfig, dryRun bool, crash crashConnectionInfo) ([]taskConfig, error) {
	...
	symbols := filterAlreadyUploadedRequestBody{SymbolIds: []crashSymbolFile{}}
	...
	if response.StatusCode != 200 {
		return tasks, fmt.Errorf("error: request to %s failed with status %d", crash.client, response.StatusCode)
	}
