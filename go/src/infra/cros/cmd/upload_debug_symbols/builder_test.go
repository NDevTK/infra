...
func initMocks(mockURL, mockKey string, responseMap map[string]string) {
	crashUrl = mockURL
	apiKey = mockKey
...
func TestGenerateConfigs(t *testing.T) {
	...
	responseBody := responseBody{Pairs: []responseStatusPair{}}
	...
	for filename, symbolStatus := range mockResponses {
		symbol := apiSymbolFileDefinitionBulkEnpoint{filename, "F4F6FA6CCBDEF455039C8DE869C8A2F40"}
		responseBody.Pairs = append(responseBody.Pairs, responseStatusPair{SymbolId: symbol, Status: symbolStatus})
	}
	...
	initMocks("google.com", "1234", map[string]string{"google.com/symbols:checkStatuses?key=1234": string(mockResponseBody)})

	tasks, err := generateConfigs(mockPaths, 0, false)
	if err != nil {
		t.Error("error: " + err.Error())
	}
...
func TestUploadSymbols(t *testing.T) {
	...
	// unique URL and key
	mockURLKeyPair := uniqueUploadInformation{UploadUrl: "crashupload.com", UploadKey: "abc"}
	mockCompleteResponse := completeUploadResponse{Result: "OK"}
	...
	initMocks("google.com", "1234", expectedResponses)
	retcode, err := uploadSymbols(tasks, 64, 2, false)
