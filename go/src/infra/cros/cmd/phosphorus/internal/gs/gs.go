...
func (w *prodDirWriter) WriteDir(ctx context.Context) error {
	...
	writeOne := func(src string, info os.FileInfo, err error) error {
		...
		if err != nil {
			...
			err = writer.Close()
			if err != nil {
				return errors.Annotate(err, "nothing written to %s, writer failed to close", dest).Err()
			}
			return errors.Reason("nothing written to %s", dest).Err()
		}
		err = writer.Close()
		if err != nil {
			return errors.Annotate(err, "writer to %s failed to close", dest).Err()
		}
		...
		if _, err := cli.Attrs(gsDest); err != nil {
