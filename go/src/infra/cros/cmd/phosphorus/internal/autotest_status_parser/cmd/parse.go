...
// hasSummary checks if human_readable_summary of the top
// test case in the stack exists.
func (s *testCaseStack) hasSummary() bool {
	if len(s.testCases) == 0 {
		return false
	}

	return len(s.testCases[len(s.testCases)-1].HumanReadableSummary) > 0
}
