...
func runGSUploadStep(ctx context.Context, authFlags authcli.Flags, r phosphorus.UploadToGSRequest, errorFile io.Writer) (string, error) {
	...
	// See crbug.com/830912 '# TEMPORARY, I HOPE HOPE HOPE', as they say
	resultsDir := filepath.Join(r.Config.Task.ResultsDir, "autoserv_test")
	localPath := filepath.Join(resultsDir, r.GetConfig().GetTask().GetSynchronousOffloadDir())
	defer func() {
