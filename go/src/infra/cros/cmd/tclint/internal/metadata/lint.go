...
func lintRemoteTestDriverName(name string) Result {
	result := Result{}
	...
	switch len(parts) {
	case 0:
		result.AppendError("%s: name must be of the form '%s/{remoteTestDriver}', found empty string", tag, remoteTestDriverCollection)
	case 1:
		if parts[0] == remoteTestDriverCollection {
			result.AppendError("%s: name must be of the form '%s/{remoteTestDriver}', missing name after '%s'", tag, remoteTestDriverCollection, remoteTestDriverCollection)
		} else {
			result.AppendError("%s: name must be of the form '%s/{remoteTestDriver}', missing prefix '%s'", tag, remoteTestDriverCollection, remoteTestDriverCollection)
		}
	case 2:
		if parts[0] != remoteTestDriverCollection {
			result.AppendError("%s: name must be of the form '%s/{remoteTestDriver}', missing prefix '%s'", tag, remoteTestDriverCollection, remoteTestDriverCollection)
	...
	default:
		result.AppendError("%s: name must be of the form '%s/{remoteTestDriver}', found trailing suffix '%s'", tag, remoteTestDriverCollection, parts[2])
	}
