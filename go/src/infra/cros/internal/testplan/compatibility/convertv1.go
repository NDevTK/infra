...
// Package compatibility provides functions for backwards compatibility with
// test platform.
...
func chooseProgramToTest(
	...
	if len(criticalPrograms) > 0 {
		logging.Infof(ctx, "Choosing between critical programs: %q", criticalPrograms)
		return prioritySelector.SelectPriority(ctx, pool, criticalPrograms)
	}

	if len(completedPrograms) > 0 {
		logging.Infof(ctx, "Choosing between completed programs: %q", completedPrograms)
		return prioritySelector.SelectPriority(ctx, pool, completedPrograms)
	}

	logging.Infof(ctx, "No completed programs found.")
	return prioritySelector.SelectPriority(ctx, pool, programs)
}
...
func getTestArtifacts(ctx context.Context, build *bbpb.Build) (*testplans.BuildPayload, error) {
	...
	if !ok {
		logging.Warningf(ctx, "artifacts.files_by_artifact not found in output properties of build %q", builderName)
		return nil, nil
	}
	...
	if !foundTestArtifact {
		logging.Warningf(ctx, "no test artifacts found for build %q", builderName)
		return nil, nil
	}
...
func buildsToBuildInfos(
	...
	for _, build := range builds {
		...
		if ok && pointless.GetBoolValue() {
			logging.Warningf(ctx, "build %q is pointless, skipping", builderName)
			continue
		}
		...
		if !ok {
			logging.Warningf(ctx, "build_target.name not found in input properties of build %q, skipping", builderName)
			continue
		}

		payload, err := getTestArtifacts(ctx, build)
		if err != nil {
			return nil, err
		}

		if payload == nil && skipIfNoTestArtifacts {
			logging.Warningf(ctx, "skipIfNoTestArtifacts set, skipping build %q", builderName)
			continue
		}
		...
		if ok {
			...
			logging.Warningf(ctx, "no BuilderConfig found for %q", builderName)
		}
		...
		if ok {
			...
			logging.Warningf(ctx, "container_building_failed not found in the output properties of build %q, assuming false (CFT containers build successfully)", builderName)
...
func coverageRuleToSuiteInfo(
	...
	// For simplicity, only allow a board variant or profile to be specified if
	...
	if len(boardVariant) > 0 || len(profile) > 0 {
		...
		chosenBoardInfo, err := chooseProgramToTest(
			ctx, pool, programs, buildTargetToBuildInfo, prioritySelector,
		)
		if err != nil {
			return nil, err
		}
		...
		logging.Infof(ctx, "chose program %q from possible programs %q", chosenProgram, programs)
	}
	...
	if rule.Critical != nil {
		...
		logging.Infof(ctx, "rule %q explicitly sets criticality %v", rule.GetName(), ruleCritical)
	}
	...
	if found && buildInfo.criticality != bbpb.Trinary_YES {
		...
		logging.Infof(ctx, "build target %q explicitly sets criticality %q", buildTarget, buildInfo.criticality)
	}
	...
	for _, hwTestPlan := range hwTestPlans {
		for _, rule := range hwTestPlan.GetCoverageRules() {
			...
			suiteInfos, err := coverageRuleToSuiteInfo(
				ctx, rule, poolAttr, programAttr, designAttr, licenseAttr, buildTargetToBuildInfo, prioritySelector, isVM,
			)
			if err != nil {
				return nil, err
			}
...
func ToCTP1(
	...
	buildInfos, err := buildsToBuildInfos(ctx, builds, builderConfigs, true)
	if err != nil {
		return nil, err
	}
	...
	buildTargetToSuiteInfos, err := extractSuiteInfos(
		ctx, rnd, hwTestPlans, vmTestPlans, dutAttributeList, boardPriorityList, buildTargetToBuildInfo,
	)
	if err != nil {
		return nil, err
	}
	...
	for _, buildInfo := range buildInfos {
		...
		if !ok {
			logging.Warningf(ctx, "no suites found for build %q, skipping tests", buildInfo.buildTarget)
			continue
		}
		...
		for _, suiteInfo := range suiteInfos {
			...
			case hw:
				...
				if _, found := hwTests[displayName]; found {
					logging.Infof(ctx, "HwTest already added: %q", hwTest)
				} else {
					hwTests[displayName] = hwTest
					logging.Infof(ctx, "added HwTest: %q", hwTest)
				}
			case tastVM:
				if suiteInfo.totalShards > 0 {
					...
					for i = 0; i < suiteInfo.totalShards; i++ {
						...
						if _, found := tastVMTests[displayName]; found {
							logging.Infof(ctx, "TastVmTest already added: %q", tastVMTest)
						} else {
							tastVMTests[displayName] = tastVMTest
							logging.Infof(ctx, "added TastVmTest %q", tastVMTest)
						}
					}
				} else {
				...
				if suiteInfo.totalShards > 0 {
					...
					for i = 0; i < suiteInfo.totalShards; i++ {
						...
						if _, found := tastGCETests[displayName]; found {
							logging.Infof(ctx, "TastGceTest already added: %q", tastGCETest)
						} else {
							tastGCETests[displayName] = tastGCETest
							logging.Infof(ctx, "added TastGceTest %q", tastGCETest)
						}
					}
				} else {
