...
func (c *UpdateDUT) TriggerRun(ctx context.Context) error {
  var err error
	if c.SatlabID == "" {
    c.SatlabID, err = satlabcommands.NewSubCommand().GetDockerHostBoxIdentifier()
		if err != nil {
			return err
		}
	}

	hostId, err := getDockerHostBoxIdentifier(c.SatlabID)
	if err != nil {
		return err
	}
