...
func TestStoreCachedCostResult(t *testing.T) {
	...
	_, readErr := controller.ReadCachedCostResult(tf.Ctx, "fake-hostname", tf.Time)
	if !datastore.IsErrNoSuchEntity(readErr) {
		t.Errorf("unexpected error in empty db: %s", readErr)
	}

	if err := controller.StoreCachedCostResult(tf.Ctx, "fake-hostname", tf.Time, &fleetcostAPI.GetCostResultResponse{
	...
	result, readErr := controller.ReadCachedCostResult(tf.Ctx, "fake-hostname", tf.Time.Add(controller.CacheTTL-time.Second))
	if readErr != nil {
	...
	_, readErr = controller.ReadCachedCostResult(tf.Ctx, "fake-hostname", tf.Time.Add(controller.CacheTTL+time.Second))
	switch {
	case readErr == nil:
