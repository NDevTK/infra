...
func deviceHostnameAndRPMOutlet(info *execs.ExecInfo, deviceType string) (string, *tlw.RPMOutlet, error) {
	...
	case "chameleon":
		...
		if err != nil {
			return "", nil, errors.Annotate(err, "rpm power cycle chameleon").Err()
		}
		return c.GetName(), c.GetRPMOutlet(), nil
	default:
		return "", nil, errors.Reason("device hostname and rpmoutlet: device type not specified or incorrect").Err()
	}
}
