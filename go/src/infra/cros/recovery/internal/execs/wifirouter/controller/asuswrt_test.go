...
func Test_hostRunNvramShowForKey(t *testing.T) {
	type args struct {
		nvramKey            string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := hostRunNvramShowForKey(context.Background(), mockSshRunner, tt.args.nvramKey)
			if (err != nil) != tt.wantErr {
				t.Errorf("hostRunNvramShowForKey() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("hostRunNvramShowForKey() got = %v, want %v", got, tt.want)
			}
		})
	}
}
