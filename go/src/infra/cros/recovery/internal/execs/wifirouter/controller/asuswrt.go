...
func hostIsAsusWrtRouter(ctx context.Context, sshRunner ssh.Runner) (bool, error) {
	...
	manufacturer, err := hostRunNvramShowForKey(ctx, sshRunner, "wps_mfstring")
	if err != nil {
		return false, err
	}
...
func hostRunNvramShowForKey(ctx context.Context, sshRunner ssh.Runner, nvramKey string) (string, error) {
...
func (c *AsusWrtRouterController) FetchAsusModel(ctx context.Context) error {
	var err error
	c.state.AsusModel, err = hostRunNvramShowForKey(ctx, c.sshRunner, "wps_modelnum")
	if err != nil {
...
func (c *AsusWrtRouterController) Model() (string, error) {
	if c.state.AsusModel == "" {
		return "", errors.Reason("AsusModel not yet retrieved from device").Err()
	}
...
func (c *AsusWrtRouterController) Features() ([]labapi.WifiRouterFeature, error) {
	if c.state == nil || c.state.AsusModel == "" {
		return nil, errors.Reason("AsusModel not yet retrieved from device").Err()
	}
