...
func isTpmFirmwareVersionCorrectExec(ctx context.Context, args *execs.RunArgs, actionArgs []string) error {
	...
	if actualTpmFirmwareVersion != expectedTpmFirmwareVersion {
		return errors.Reason(`Unexpected "tpm_fwver" value: %s, expected: %s. This error may cause firmware provision fail due to the rollback protection`, actualTpmFirmwareVersion, expectedTpmFirmwareVersion).Err()
	}
	return nil
}
