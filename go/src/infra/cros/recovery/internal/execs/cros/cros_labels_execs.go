...
// updateDeviceSKUExec will update the DUT's SKU label from either inventory
...
func updateDeviceSKUExec(ctx context.Context, args *execs.RunArgs, actionArgs []string) error {
	...
	if deviceSkuLabel != "" {
		log.Info(ctx, "Device SKU found in the inventory: %s. Skipping update.", deviceSkuLabel)
		return nil
	}
