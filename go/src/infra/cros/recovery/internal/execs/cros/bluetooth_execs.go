...
func auditBluetoothExec(ctx context.Context, args *execs.RunArgs, actionArgs []string) error {
	...
	exitCode := errors.GetTags(err)[tagKey]
	if exitCode == -1 || exitCode == -2 || exitCode == 127 {
		return errors.Reason("audit bluetooth: bluetooth hardware state unspecified").Err()
	}
	...
	return errors.Reason("audit bluetooth: bluetooth hardware state not detected").Err()
}
