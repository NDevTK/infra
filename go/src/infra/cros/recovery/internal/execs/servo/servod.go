...
func (s newString) isEqual(ctx context.Context, expectedValue string) error {
	log.Debug(ctx, "Is Equal (String): receiver value: %q, expectedValue: %q", s, expectedValue)
	if expectedValue != string(s) {
		return errors.Reason("is equal (string): expected value %s is not equal to actual value %s", expectedValue, s).Err()
	}
	return nil
}
...
func (i newInt) isEqual(ctx context.Context, expectedValue string) error {
	...
	log.Debug(ctx, "Is Equal (Int32): receiver value: %d, expectedValue: %q", i, expectedInt)
	if err != nil {
		return errors.Annotate(err, "is equal").Err()
	}
	if newInt(expectedInt) != i {
		return errors.Reason("is equal (int32): expected value %d is not equal to actual value %d", expectedValue, i).Err()
	}
	return nil
}
...
func (f newDouble) isEqual(ctx context.Context, expectedValue string) error {
	...
	log.Debug(ctx, "Is Equal (Double): receiver value: %f, expectedValue: %s", f, expectedValue)
	if err != nil {
		return errors.Annotate(err, "is equal").Err()
	}
	if newDouble(expectedDouble) != f {
		return errors.Reason("is equal (double): expected value %f is not equal to actual value %f", expectedValue, f).Err()
	}
	return nil
}
...
func (b newBool) isEqual(ctx context.Context, expectedValue string) error {
	...
	log.Debug(ctx, "Is Equal (Bool): receiver value: %t, expectedValue: %s", b, expectedValue)
	if err != nil {
		return errors.Annotate(err, "is equal").Err()
	}
	if newBool(expectedBool) != b {
		return errors.Reason("is equal (bool): expected value %t is not equal to actual value %t", expectedValue, b).Err()
	}
	return nil
}
