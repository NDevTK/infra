...
func chameleonCheckAudioboxJackpluggerExec(ctx context.Context, info *execs.ExecInfo) error {
	...
	if info.GetChromeos().GetChameleon().Audioboxjackpluggerstate == tlw.Chameleon_AUDIOBOX_JACKPLUGGER_NOT_APPLICABLE {
		log.Debugf(ctx, "Already marked as N/A - Skip and do nothing.")
		return nil
	}
