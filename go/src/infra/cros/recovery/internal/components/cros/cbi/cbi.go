...
func parseBytesFromCBIContents(cbiContents string, numBytesToRead int) ([]string, error) {
	...
	if len(hexBytes) != numBytesToRead {
		return nil, errors.Reason("read the incorrect amount of bytes from CBI. Intended to read %d bytes but read %d bytes instead. CBI Contents found: %s", numBytesToRead, len(hexBytes), cbiContents).Err()
	}
	return hexBytes, nil
}
