...
// ensureFileExists checks whether the file 'f' exists, or not. If it
// does not exist, the function returns an error.
func checkFileExists(ctx context.Context, f string) error {
	s, err := os.Stat(f)
	if err != nil {
		if os.IsNotExist(err) {
			errors.Annotate(err, "check file exists: file %q does not exist", f).Err()
		}
		// This means that 'err' is not known to report whether or not
		// the file or directory already exists. Hence we cannot
		// proceed with checking whether the file exists or not.
		return errors.Annotate(err, "check file exists: cannot determine if %q exists", f).Err()
	}
	if s.IsDir() {
		return errors.Annotate(err, "check file exists: %q is a directory, not a file", f).Err()
	}
	return nil
}
