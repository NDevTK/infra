...
func wifiRouterRepairPlan() *Plan {
	return &Plan{
		...
		Actions: map[string]*Action{
			...
			"Is AsusWrt": {
				Docs: []string{
					"Checks if the router's device type is WifiRouterDeviceType_WIFI_ROUTER_DEVICE_TYPE_ASUSWRT.",
				},
				ExecName: "wifi_router_device_type_in_list",
				ExecExtraArgs: []string{
					"device_types:WifiRouterDeviceType_WIFI_ROUTER_DEVICE_TYPE_ASUSWRT",
				},
				RunControl:    RunControl_RUN_ONCE,
				MetricsConfig: &MetricsConfig{UploadPolicy: MetricsConfig_SKIP_ALL},
			},
