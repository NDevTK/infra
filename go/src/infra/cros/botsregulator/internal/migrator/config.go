...
type boardToModelsToPercentage map[string]map[string]int32
	...
	for _, override := range config.Overrides {
		...
		if override.Model == "*" {
			...
			if _, ok := bma[override.Board][override.Model]; !ok {
				bma[override.Board][override.Model] = override.Percentage
			} else {
