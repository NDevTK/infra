...
var (
	wiringPort = flag.Int("wiring-port", 7152, "port of the TLW server to use for tlslib integration tests")
	wiringHost = flag.String("wiring-host", "127.0.0.1", "host name/IP of the TLW server to use for tlslib integration tests")
...
// environment.
// The requirements are: 1) a workable TLW implementation; 2) locally
// installed `gsutil` and authorized to "gs://chromeos-image-archive"; 3) a
// SSH-able DUT.
// Command line to run:
// go test -wiring-port 7152 -dut <DUT_name> ./...
// or we can use "-run TestFakeOmahaIntegration" instead of "./..." to just run
// this test.
func TestFakeOmahaIntegration(t *testing.T) {
	...
	if info, err := os.Stat(nebraska.NebraskaScript); os.IsNotExist(err) || info.IsDir() {
		t.Skipf("skip integration test due to %q isn't exist or isn't a normal file", nebraska.NebraskaScript)
	}
	connTlw, err := grpc.Dial(fmt.Sprintf("%s:%d", *wiringHost, *wiringPort), grpc.WithInsecure())
	if err != nil {
		t.Skip(fmt.Sprintf("skip due to no connections to TLW service (:%d)", *wiringPort))
	}
	...
	t.Cleanup(func() {
		t.Logf("Delete the fake Omaha created")
