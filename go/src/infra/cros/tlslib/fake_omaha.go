...
func (s *Server) CreateFakeOmaha(ctx context.Context, req *tls.CreateFakeOmahaRequest) (*tls.FakeOmaha, error) {
	...
	if len(payloads) == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "CreateFakeOmaha: no payloads specified")
	}
	...
	payloadsProvider, err := s.cacheForDut(ctx, gsPathPrefix, dutName)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "CreateFakeOmaha: failed to get cache payload URL: %s", err)
	}
	n, err := nebraska.NewServer(nebraska.NewEnvironment(), gsPathPrefix, payloads, payloadsProvider)
	if err != nil {
