...
func (e fakeEnv) DownloadMetadata(gsPathPrefix string, payloads []*tls.FakeOmaha_Payload) (string, error) {
	return "", nil
}

func (e fakeEnv) DownloadNebraska(string) error {
	return nil
}
...
func TestNebraska(t *testing.T) {
	t.Parallel()
	portWant := 12345
	port := []byte(fmt.Sprintf("%d", portWant))
	e := fakeEnv{
	...
	n, err := NewServer(e, "gs://", []*tls.FakeOmaha_Payload{}, "http://cache-server/update")
	if err != nil {
...
func TestNebraskaTimeoutOnPort(t *testing.T) {
	...
	_, err := NewServer(e, "gs://", []*tls.FakeOmaha_Payload{}, "http://cache-server/update")
	if err == nil {
...
func TestDownloadMetadata(t *testing.T) {
	...
	for _, tc := range tests {
		e.DownloadMetadata(gsPrefix, tc.types)
		w := []string{"gsutil cp"}
