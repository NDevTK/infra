...
// Process is a process of nebraska.py.
...
type Environment interface {
	...
	// It's callers' responsibility to remove the temporary directory after use.
	DownloadMetadata(ctx context.Context, gsPathPrefix string, payloads []*tls.FakeOmaha_Payload) (string, error)
	StartNebraska(*Server) (Process, error)
}
...
func (n *Server) start(ctx context.Context, gsPathPrefix string, payloads []*tls.FakeOmaha_Payload) error {
	...
	if n.proc, err = n.env.StartNebraska(n); err != nil {
		return err
	}
...
func (e env) StartNebraska(n *Server) (Process, error) {
	cmdline := n.cmdline()
...
func (p proc) Stop() error {
	if err := unix.Kill(p.cmd.Process.Pid, syscall.SIGTERM); err != nil {
		return fmt.Errorf("stop Nebraska (pid: %d): %s", p.cmd.Process.Pid, err)
	}
	return nil
}
func (p proc) Pid() int {
