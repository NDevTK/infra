...
func getAvailableFirmwareVersion(c *ssh.Client) (string, error) {
	out, err := runCmdOutput(c, "chromeos-firmwareupdate --manifest")
	if err != nil {
...
func getCurrentFirmwareVersion(c *ssh.Client) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("getFirmwareVersion: failed to read current system firmware, %s", err)
	}
	return out, nil
}
...
func (p *provisionState) updateFirmware(ctx context.Context) error {
	if err := runCmd(p.c, "/usr/sbin/chromeos-firmwareupdate --wp=1 --mode=autoupdate"); err != nil {
