...
func ExtractAnalyzerName(workerName string) (string, error) {
	parts := strings.Split(workerName, workerSeparator)
	if len(parts) != 2 {
...
// GetAnalyzerRunsForWorkflowRun returns AnalyzerRun entities with a ancestor WorkflowRun with the provided run ID.
func GetAnalyzerRunsForWorkflowRun(c context.Context, runID int64) ([]*track.AnalyzerRun, error) {
	runKey
