...
func collect(c context.Context, req *admin.CollectRequest,
	...
	if taskResult.OutputsRef == nil || taskResult.OutputsRef.Isolated == "" {
		...
		}.Errorf(c, "Task done without output.")
		return fmt.Errorf("task done but with no isolated output")
	}
	...
	b, err := proto.Marshal(&admin.WorkerDoneRequest{
		RunId:              req.RunId,
		Worker:             req.Worker,
		IsolatedOutputHash: taskResult.OutputsRef.Isolated,
		Provides:           w.Provides,
		State:              workerState,
	})
	if err != nil {
	...
	isolatedInput, err := isolator.LayerIsolates(
		c, wf.IsolateServer, req.IsolatedInputHash, taskResult.OutputsRef.Isolated)
	if err != nil {
