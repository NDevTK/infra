...
func workerDone(c context.Context, req *admin.WorkerDoneRequest, isolator common.IsolateAPI) error {
	...
	runKey := ds.NewKey(c, "WorkflowRun", "1", 0, requestKey)
	...
	resultsStr, err := isolator.FetchIsolatedResults(c, workflowRun.IsolateServerURL, req.IsolatedOutputHash)
	if err != nil {
	...
	if err := ds.GetAll(c, ds.NewQuery("WorkerRunResult").Ancestor(analyzerKey), &workerResults); err != nil {
		return fmt.Errorf("failed to retrieve WorkerRunResult entities: %v", err)
	}
