...
func filterByWhitelist(ctx context.Context, whitelistedGroups []string, changes []gr.ChangeInfo) []gr.ChangeInfo {
	...
	for _, c := range changes {
		...
		if !ok {
			...
			if err != nil {
				logging.WithError(err).Errorf(ctx, "Failed to create identity for %s", email)
				continue
			}
			authOK, err := authDB.IsMember(ctx, ident, whitelistedGroups)
			if err != nil {
				logging.WithError(err).Errorf(ctx, "Failed to check auth for %s", email)
			}
			whitelisted = authOK
			if !whitelisted {
				logging.Infof(ctx, "Owner %s is not whitelisted, skipping Analyze", email)
			}
