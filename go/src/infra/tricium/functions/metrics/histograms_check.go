...
const (
	...
	allRemovedHistogramInfo      = `[INFO] The following histograms have been removed in this CL: %s.`
)
...
func analyzeHistogramFile(f io.Reader, filePath, prevDir string, filesChanged *diffsPerFile, singletonEnums stringset.Set, obsoletedHistograms map[string]bool, histogramStatus map[string]histogramStatus) []*tricium.Data_Comment {
	...
	allComments = append(allComments, generateCommentsForRemovedHistograms(filePath, newHistograms, oldHistograms, newVariants, oldVariants, obsoletedHistograms, histogramStatus)...)
...
func generateCommentsForRemovedHistograms(path string, newHistograms map[string]*histogram, oldHistograms map[string]*histogram, newVariants map[string]*variants, oldVariants map[string]*variants, obsoletedHistograms map[string]bool, histogramStatus map[string]histogramStatus) []*tricium.Data_Comment {
	var comments []*tricium.Data_Comment
	...
	for _, removedHistogram := range allRemovedHistograms {
		if _, present := histogramStatus[removedHistogram]; present {
			histogramStatus[removedHistogram] -= 1
		} else {
			histogramStatus[removedHistogram] = REMOVE
		}
	...
	for _, addedHistogram := range newHistogramNames.Difference(oldHistogramNames).ToSlice() {
		if _, present := histogramStatus[addedHistogram]; present {
			histogramStatus[addedHistogram] += 1
		} else {
			histogramStatus[addedHistogram] = ADD
		}
	}
	return comments
}
...
// expandHistograms generates a map containing all histograms keyed by their names after expansion.
