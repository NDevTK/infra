...
const (
	...
	firstOwnerTeamError = "First owner must be an individual, not a team"
)
...
// NewLineNum is a constructor for creating a LineNum struct with defaultLineNum
...
func checkHistogram(path string, histBytes []byte, tagLineNums *LineNum) []*tricium.Data_Comment {
	...
	xml.Unmarshal(histBytes, &histogram)

	var comments []*tricium.Data_Comment
	if comment := checkNumOwners(path, histogram.Owners, tagLineNums.OwnerLineNum); comment != nil {
		comments = append(comments, comment)
	}
	if comment := checkNonTeamOwner(path, histogram.Owners, tagLineNums.OwnerLineNum); comment != nil {
		comments = append(comments, comment)
	}
	return comments
}

func checkNumOwners(path string, owners []string, ownerLineNum int) *tricium.Data_Comment {
	if len(owners) <= 1 {
...
func checkNonTeamOwner(path string, owners []string, ownerLineNum int) *tricium.Data_Comment {
	if len(owners) > 0 && strings.Contains(owners[0], "team") {
