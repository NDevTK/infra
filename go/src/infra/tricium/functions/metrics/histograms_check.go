...
const (
	...
	allRemovedHistogramInfo      = `[INFO] %s has been removed in this CL.`
)
...
func analyzeHistogramFile(f io.Reader, filePath, prevDir string, filesChanged *diffsPerFile, singletonEnums stringset.Set, obsoletedHistograms map[string]bool, removedHistogramSet map[string]bool) []*tricium.Data_Comment {
	...
	allComments = append(allComments, generateCommentsForRemovedHistograms(filePath, newHistograms, oldHistograms, newVariants, oldVariants, obsoletedHistograms, removedHistogramSet)...)
...
// 5. A amp containing all variants keyed by their names in the file.
...
func generateCommentsForRemovedHistograms(path string, newHistograms map[string]*histogram, oldHistograms map[string]*histogram, newVariants map[string]*variants, oldVariants map[string]*variants, obsoletedHistograms map[string]bool, removedHistogramSet map[string]bool) []*tricium.Data_Comment {
	var comments []*tricium.Data_Comment
	...
	for _, removedHistogram := range allRemovedHistograms {
		removedHistogramSet[removedHistogram] = true
