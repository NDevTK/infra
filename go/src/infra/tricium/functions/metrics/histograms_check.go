...
func expandHistograms(hists map[string]*histogram, variants map[string]*variants) stringset.Set {
	...
	for name := range hists {
		hist := hists[name]
		if hist.Obsolete != "" || hasExpired(hist, 0) {
			continue
		}
		if hist.Tokens == nil {
...
// hasExpired returns whether the given histogram has been expired for more than the grace period.
func hasExpired(hist *histogram, gracePeriod int) bool {
	if hist.Expiry != "" {
		if inputDate, _, _, ok := getExpiryDate(hist.Expiry); ok {
			dateDiff := int(inputDate.Sub(now()).Hours() / 24)
			return dateDiff < -1*gracePeriod
		}
	}
	return false
}
