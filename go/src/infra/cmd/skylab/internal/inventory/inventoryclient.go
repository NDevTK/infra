...
func (client *inventoryClientV2) UpdateDUT(ctx context.Context, newSpecs *inventory.CommonDeviceSpecs) error {
	// Copy from https://source.corp.google.com/chromium_infra/go/src/infra/appengine/crosskylabadmin/app/frontend/inventory/clientv2.go;l=145
	devicesToUpdate, labstations, _, err := invV2Api.ImportFromV1DutSpecs([]*inventory.CommonDeviceSpecs{newSpecs})
	if err != nil {
		return errors.Annotate(err, "update dut: convert DUT spec").Err()
	}
	...
	if err != nil {
		return errors.Annotate(err, "update dut: update setup configs").Err()
	}
