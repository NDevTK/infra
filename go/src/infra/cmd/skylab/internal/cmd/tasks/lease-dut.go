...
func (c *leaseDutRun) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	var matcher utils.HostMatcher = &utils.Model{Data: c.model}
	host := ""
	if hasOneHostname {
		...
		matcher = &utils.Hostname{Data: host}
	}
	...
	id, err := creator.LeaseTask(ctx, matcher, int(leaseDuration.Seconds()), c.leaseReason)
	if err != nil {
		return err
	}
	fmt.Fprintf(a.GetOut(), "Created lease task for %q: %s\n", matcher.Tag(), swarming.TaskURL(e.SwarmingService, id))
