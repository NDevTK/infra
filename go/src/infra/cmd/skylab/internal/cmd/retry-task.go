...
func (c *retryTasksRun) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	for i, ID := range originalTaskIDs {
		...
		newRequest, err := createRetryRequest(request, e)
		if err != nil {
			return errors.Annotate(err, fmt.Sprintf("retry task %s", ID)).Err()
		}
...
// createRetryRequest creates a new request to retry a previous Skylab task (by changing nothing but
// the annotation URL).
func createRetryRequest(original *swarming.SwarmingRpcsTaskRequest, e site.Environment) (*swarming.SwarmingRpcsNewTaskRequest, error) {
	...
	return newTaskRequest(original.Name, original.Tags, slices, 1), nil
}
