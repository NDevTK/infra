...
func (c *testCommonFlags) verifyFleetTestsPolicy(ctx context.Context, ufsClient Client,
	...
	if err == nil && isPublicTestResponse.Status == ufsapi.CheckFleetTestsPolicyResponse_OK {
	...
	for _, model := range c.models {
		for _, testName := range testNames {
			...
			if err == nil && resp.Status == ufsapi.CheckFleetTestsPolicyResponse_OK {
				results.anyValidTests = true
				validModelsMap[model] = true
				validTestNamesMap[testName] = true
				continue
			}
			results.testValidationErrors = append(results.testValidationErrors, resp.StatusMessage)
