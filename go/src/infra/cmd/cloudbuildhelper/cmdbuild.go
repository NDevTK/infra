...
The "build" command works in multiple steps:
  ...
  3. Calculates SHA256 of the tarball and uses it to construct a Google Storage
     ...
     tarball's metadata to find a canonical tag of some previous image built
     from this tarball. If it exists, returns this canonical tag as a result.
...
// maybeReuseExistingImage search for an image with requested canonical tag.
//
// Returns:
