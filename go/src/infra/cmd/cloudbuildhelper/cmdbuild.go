...
func reportSuccess(ctx context.Context, r *buildResult) {
	...
	if img.CanonicalTag == "" {
		logging.Infof(ctx, "    Name:   %s", img.Image)
		logging.Infof(ctx, "    Digest: %s", img.Digest)
		logging.Infof(ctx, "    View:   https://%s@%s", img.Image, img.Digest)
	} else {
		logging.Infof(ctx, "    Name:   %s:%s", img.Image, img.CanonicalTag)
		logging.Infof(ctx, "    Digest: %s", img.Digest)
		logging.Infof(ctx, "    View:   https://%s:%s", img.Image, img.CanonicalTag)
	}
}
