...
type recoveryLogger struct {
	log logging.Logger
	// indenter holds indent for each message.
	indenter rlogger.Indenter
...
func (l *recoveryLogger) Debug(format string, args ...interface{}) {
	l.log.LogCall(logging.Debug, l.callDepth, l.addIndent(format), args)
}

// Info is like Debug, but logs at Info level.
func (l *recoveryLogger) Info(format string, args ...interface{}) {
	l.log.LogCall(logging.Info, l.callDepth, l.addIndent(format), args)
}

// Warning is like Debug, but logs at Warning level.
func (l *recoveryLogger) Warning(format string, args ...interface{}) {
	l.log.LogCall(logging.Warning, l.callDepth, l.addIndent(format), args)
}

// Error is like Debug, but logs at Error level.
func (l *recoveryLogger) Error(format string, args ...interface{}) {
	l.log.LogCall(logging.Error, l.callDepth, l.addIndent(format), args)
}

// Indenter provides access to logger intender.
func (l *recoveryLogger) GetIndenter() rlogger.Indenter {
	if l.indenter == nil {
		l.indenter = rlogger.NewIndenter()
	}
	return l.indenter
}

// Apply indent to the string.
func (l *recoveryLogger) addIndent(v string) string {
	indent := rlogger.GetIntentString(l.GetIndenter(), "  ")
	return indent + v
}
