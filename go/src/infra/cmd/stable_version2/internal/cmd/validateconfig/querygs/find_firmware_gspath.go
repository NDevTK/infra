...
func (r *Reader) FindFirmwarePath(board string, milestone int, tip int, branch int, branchBranch string) (*FindFirmwarePathResult, error) {
	...
	for _, m := range milestones {
		candidates = append(candidates, gs.Path(fmt.Sprintf("gs://chromeos-image-archive/%s-release/R%d-%d.%d.%s/firmware_from_source.tar.bz2", board, m, tip, branch, branchBranch)))
		images = append(images, fmt.Sprintf("%s-release/R%d-%d.%d.%s", board, m, tip, branch, branchBranch))
		candidates = append(candidates, gs.Path(fmt.Sprintf("gs://chromeos-image-archive/%s-firmware/R%d-%d.%d.%s/firmware_from_source.tar.bz2", board, m, tip, branch, branchBranch)))
		images = append(images, fmt.Sprintf("%s-firmware/R%d-%d.%d.%s", board, m, tip, branch, branchBranch))
	}
