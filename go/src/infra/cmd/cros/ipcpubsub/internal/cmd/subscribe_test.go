...
type dummySlowSubscription struct {
	dummyIDlessSubscription
	delay time.Duration
}

func newSlowSubscription(timeString string) *dummySlowSubscription {
	delay, _ := time.ParseDuration(timeString)
	return &dummySlowSubscription{
		dummyIDlessSubscription: *newDummySubscription(),
		delay:                   delay,
	}
}

func (s *dummySlowSubscription) inputMessages(msgs []pubsublib.Message) {
	for _, msg := range msgs {
		time.Sleep(s.delay)
		go func(m pubsublib.Message) {
			s.stream <- m
...
func TestRejectMessagesWithoutAttrs(t *testing.T) {
	...
	// Therefore, run five times and pass only if all pass
	// Theoretical: If message ordering is random (probably false), this has a flake rate of 0.032%
	// Experimental: go test -run TestRejectMessagesWithoutAttrs -count 100000 passes reliably
	for i := 0; i < 5; i++ {
		sub := newDummySubscription()
		sub.SetFilter(ctx, &foobarFilter)
		published := []pubsublib.Message{&msg1, &msg1, &msg2, &msg2, &msg3}
		sub.inputMessages(published)
		bodies, err := Subscribe(ctx, sub, 1)
		if err != nil {
