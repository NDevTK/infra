...
Add a VM by parameters.`,
	CommandRun: func() subcommands.CommandRun {
		...
		c.Flags.StringVar(&c.newSpecsFile, "new-json-file", "", cmdhelp.VMFileText)
...
func (c *addVM) validateArgs() error {
	...
	if c.newSpecsFile != "" {
		...
		if c.vmName == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file') is specified, so '-name' is required.")
		}
		if c.macAddress == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file') is specified, so '-mac-address' is required.")
		}
		if c.osVersion == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file') is specified, so '-os-version' is required.")
		}
	}
	return nil
}
