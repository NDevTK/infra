...
var AddHostCmd = &subcommands.Command{
	...
	CommandRun: func() subcommands.CommandRun {
		...
		c.Flags.StringVar(&c.newSpecsFile, "new-json-file", "", cmdhelp.MachineLSEFileText)
		c.Flags.BoolVar(&c.interactive, "interactive", false, "enable interactive mode for input")
...
func (c *addHost) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if err := verifyLSEPrototype(ctx, ic, &machinelse); err != nil {
		return err
	}
...
func verifyLSEPrototype(ctx context.Context, ic ufsAPI.FleetClient, lse *ufspb.MachineLSE) error {
	if lse.GetChromeBrowserMachineLse() != nil {
		if !ufsUtil.IsInBrowserLab(lse.GetMachineLsePrototype()) {
			return errors.New(fmt.Sprintf("Prototype %s doesn't belong to browser lab", lse.GetMachineLsePrototype()))
		}
		if resp, err := ic.GetMachineLSEPrototype(ctx, &ufsAPI.GetMachineLSEPrototypeRequest{
			Name: ufsUtil.AddPrefix(ufsUtil.MachineLSEPrototypeCollection, lse.GetMachineLsePrototype()),
		}); err == nil {
			for _, v := range resp.GetVirtualRequirements() {
				if v.GetVirtualType() == ufspb.VirtualType_VIRTUAL_TYPE_VM {
					...
					if c < v.GetMin() || c > v.GetMax() {
						return errors.New(fmt.Sprintf("Prototype %s is not matched to the vm capacity %d", lse.GetMachineLsePrototype(), c))
					}
				}
			}
		} else {
...
func (c *addHost) validateArgs() error {
	...
	if c.newSpecsFile == "" && !c.interactive {
		if c.hostName == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive') is specified, so '-name' is required.")
		}
		if c.machineName == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive') is specified, so '-machine' is required.")
		}
		if c.prototype == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive') is specified, so '-prototype' is required.")
		}
	}
	return nil
}
