...
var AddDracCmd = &subcommands.Command{
	...
	CommandRun: func() subcommands.CommandRun {
		...
		c.Flags.StringVar(&c.newSpecsFile, "new-json-file", "", cmdhelp.DracFileText)
		...
		c.Flags.IntVar(&c.port, "port", 0, "the port of the switch that this drac is connected to")
		return c
	},
}
...
func (c *addDrac) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if c.interactive {
		...
		if c.newSpecsFile != "" {
			...
			if drac.GetName() == "" {
				return errors.New("drac name cannot be empty in json file")
			}
			if drac.GetMacAddress() == "" {
				return errors.New("macAddress cannot be empty in json file")
			}
			if drac.GetSwitchInterface().GetSwitch() == "" {
				return errors.New("switch name cannot be empty in json file")
			}
			if drac.GetSwitchInterface().GetPort() == 0 {
				return errors.New("port cannot be 0 in json file")
			}
		} else {
			c.parseArgs(&drac)
		}
...
func (c *addDrac) validateArgs() error {
	...
	if c.newSpecsFile == "" && !c.interactive {
		...
		if c.switchName == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive') is specified, so '-switch' is required.")
		}
		if c.macAddress == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive') is specified, so '-mac-address' is required.")
		}
		if c.machineName == "" {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nMachine name (-machine) is required.")
		}
		if c.port == 0 {
			return cmdlib.NewUsageError(c.Flags, "Wrong usage!!\nNo mode ('-new-json-file' or '-interactive'), so '-port' is required.")
		}
	}
	return nil
}
