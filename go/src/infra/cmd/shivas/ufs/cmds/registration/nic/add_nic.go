...
var AddNicCmd = &subcommands.Command{
	...
	CommandRun: func() subcommands.CommandRun {
		...
		c.Flags.StringVar(&c.newSpecsFile, "new-json-file", "", cmdhelp.NicFileText)
		...
		c.Flags.IntVar(&c.port, "port", 0, "the port of the switch that this nic is connected to")
		return c
	},
}
...
func (c *addNic) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if c.interactive {
		...
		if c.newSpecsFile != "" {
			...
			if nic.GetName() == "" {
				return errors.New("nic name cannot be empty in json file")
			}
			...
			if nic.GetSwitchInterface().GetPort() == 0 {
