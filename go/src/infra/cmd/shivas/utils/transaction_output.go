...
// PrintExistingDevboardLSE prints the old attached device host in update/delete operations.
func PrintExistingDevboardLSE(ctx context.Context, ic ufsAPI.FleetClient, name string) (*ufspb.MachineLSE, error) {
	res, err := ic.GetMachineLSE(ctx, &ufsAPI.GetMachineLSERequest{
		Name: ufsUtil.AddPrefix(ufsUtil.MachineLSECollection, name),
	})
	if err != nil {
		return nil, errors.Annotate(err, "Failed to get devboard LSE").Err()
	}
	if res == nil {
		return nil, errors.Reason("The returned resp is empty").Err()
	}
	if res.GetChromeosMachineLse().GetDeviceLse().GetDevboard() == nil {
		return nil, errors.Reason("Host %s is not a devboard LSE.", name).Err()
	}
	res.Name = ufsUtil.RemovePrefix(res.Name)
	fmt.Println("The attached device host before delete/update:")
	PrintProtoJSON(res, !NoEmitMode(false))
