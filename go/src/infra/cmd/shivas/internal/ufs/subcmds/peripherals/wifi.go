...
func (c *manageWifiCmd) run(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if c.isCSVUpdate {
		...
		for dutName := range c.routersMap {
			err = runSingleDut(ctx, client, dutName, c)
			if err != nil {
				return errors.Annotate(err, "update csv error %s", dutName).Err()
			}
		}
	} else {
		return runSingleDut(ctx, client, c.dutName, c)
	}

	return nil
}

func runSingleDut(ctx context.Context, client rpc.FleetClient, dutName string, c *manageWifiCmd) error {
