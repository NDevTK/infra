...
func (c *managePasitHostCmd) getModels() []string {
	var models []string

	// Empty host topology has no features.
	if c.hostObj == nil {
		return models
	}
	...
	for _, d := range c.hostObj.GetDevices() {
		// We don't need to schedule based on the model for switches or DUTs so ignore
		if d.GetType() == labapi.PasitHost_Device_DUT || d.GetType() == labapi.PasitHost_Device_SWITCH_FIXTURE {
			continue
		}

		m := d.GetModel()
		if m != "" && !seen[m] {
			// Keep trimmed and upper for consistency.
			models = append(models, strings.ToUpper(strings.TrimSpace(m)))
			seen[m] = true
		}
	}
	return models
