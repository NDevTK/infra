...
// 1 for ID and 7 for location
const lenOfAssetStringList = 8
...
func (lstats *LogStats) populateLogFile() error {
	...
	if err == nil {
		...
		return errors.Annotate(err, fmt.Sprintf("fail to read file %s", lstats.LogPath)).Err()
	}
...
func (lstats *LogStats) populateResFile() error {
	...
	if err == nil {
		...
		return errors.Annotate(err, fmt.Sprintf("fail to read file %s", lstats.ResPath)).Err()
	}
...
// PrintLogStatsAndResult prints the stats and results for a sepecified audit scan run.
