...
func (s *Store) retrieveLocalState() error {
	...
	if os.IsNotExist(err) {
		// If the file not exists we marshal the nil value LocalDUTState
		// here, so the later unmarshal can correctly initilize LocalDUTState
		// with empty value.
		data, err = swmbot.Marshal(&s.LocalDUTState)
		if err != nil {
