...
func luciferFlow(ctx context.Context, a *args, i *harness.Info, annotWriter writeCloser) error {
	...
	/*
		We want run tasks for scheudling in sequential instead of concurrect to avoid:
			...
			2.  Unexpected pressure to servohost. DUTs under a scheduling unit are
				...
				may cause them timeout or failure due to servo-usb or labstation
				performance limitation.
	*/
	for _, dh := range i.DUTs {
		ta := lucifer.TaskArgs{
			AbortSock:  filepath.Join(dh.DUTResultsDir, "abort_sock"),
			GCPProject: gcpProject,
			ResultsDir: dh.DUTResultsDir,
			LogDogFile: fifoPath,
		}
