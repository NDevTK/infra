...
func TestCreation(t *testing.T) {
	...
	p, cleanup := createUnreadableDir(t, root)
	defer cleanup()
	if err := d.Discover(p); err == nil {
		t.Errorf("Load() did not return error on unreadable path")
	}
}
...
func createUnreadableDir(t *testing.T, root string) (string, func()) {
	t.Helper()
	dir := filepath.Join(root, "unreadable")
	if err := os.Mkdir(dir, 000); err != nil {
		panic(err)
	}
	return dir, func() {
		_ = os.Chmod(dir, 0755)
...
func createDummyFileOrDie(t *testing.T, path ...string) string {
	...
	if err := ioutil.WriteFile(filepath.Join(dir, file), []byte("fake control"), 0644); err != nil {
		panic(err)
	}
