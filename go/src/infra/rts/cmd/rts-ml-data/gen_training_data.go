...
const cvs_header = "ResultId,TestName,TestId,FileName,VariantHash,SixMonthRunCount,SixMonthFailCount,OneMonthRunCount,OneMonthFailCount,OneWeekRunCount,OneWeekFailCount,Distance,UseDistance,Failed"
...
func (r *genTraingData) openCsv() error {
	...
	if r.out != "" {
		...
		r.file, err = os.OpenFile(r.in, os.O_RDWR|os.O_APPEND, 0666)
		if err != nil {
			return err
		}
	}
...
func (r *genTraingData) writeCsv(row bqRow) {
	...
	// Training will handle missing values. Don't need to figure out average.
	// This lets us continuously append without having to keep track of an
	// average (which will be used for missing values in the model)
	if row.UseDistance {
