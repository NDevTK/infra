...
func (r *genTraingData) openCsv() error {
	...
	outfile := r.in
	if outfile == "" {
		//in was not declared, we're creating the file
		outfile = r.out
		...
		fmt.Fprintf(r.file, "ResultId,TestName,TestId,FileName,VariantHash,SixMonthRunCount,SixMonthFailRate,OneMonthRunCount,OneMonthFailRate,OneWeekRunCount,OneWeekFailRate,Distance,UseDistance,Expected,\n")
	} else {
		//in was declared, we're appending the file
		r.file, err = os.OpenFile(outfile, os.O_RDWR|os.O_APPEND, 0666)
		if err != nil {
			return err
		}
	}
...
func (r *genTraingData) writeCsv(row bqRow) {
	fmt.Fprintf(r.file, "%v,", row.ResultId)
	fmt.Fprintf(r.file, "%v,", row.TestName)
	fmt.Fprintf(r.file, "%v,", row.TestId)
	fmt.Fprintf(r.file, "%v,", row.FileName)
	fmt.Fprintf(r.file, "%v,", row.VariantHash)
	fmt.Fprintf(r.file, "%v,", row.SixMonthRunCount)
	fmt.Fprintf(r.file, "%f,", row.SixMonthFailRate)
	fmt.Fprintf(r.file, "%v,", row.OneMonthRunCount)
	fmt.Fprintf(r.file, "%f,", row.OneMonthFailRate)
	fmt.Fprintf(r.file, "%v,", row.OneWeekRunCount)
	fmt.Fprintf(r.file, "%f,", row.OneWeekFailRate)
