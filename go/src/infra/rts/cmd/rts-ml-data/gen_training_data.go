	...
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			r.Flags.IntVar(&r.downSample, "down-sample", 1000, text.Doc(`
				...
				down sampling
			`))
			r.Flags.BoolVar(&r.ignorePassedJobs, "ignore-passed-jobs", false,
				"Whether or not to ignore results from builds that passed")
			r.Flags.BoolVar(&r.onlyFailures, "only-failures", false, text.Doc(`
...
func (r *genTraingData) queryResults(ctx context.Context, bqClient *bigquery.Client) ([]bqRow, error) {
	...
	q.Parameters = []bigquery.QueryParameter{
		...
		{Name: "ignorePassedJobs", Value: r.ignorePassedJobs},
		{Name: "onlyFailures", Value: r.onlyFailures},
	}
...
bb_tryjobs AS (
	...
	WHERE create_time BETWEEN TIMESTAMP_SUB(@runDate, INTERVAL 1 DAY) AND TIMESTAMP_ADD(@runDate, INTERVAL 1 DAY)
		...
		AND (not @ignorePassedJobs or b.status = "FAILURE")
),
...
WHERE
	...
	AND (NOT @onlyFailures OR rdb.all_unexpected)
