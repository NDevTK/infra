...
// TemplateService bundles a backend.Service with its ServiceIncident children
...
func dashboard(ctx *router.Context) {
	...
	service, err := backend.GetService(c, "monorail")
	if err != nil {
		http.Error(w, "Failed to query datastore, see logs", http.StatusInternalServerError)
		return
	}
	incidents, _ := backend.GetServiceIncidents(c, service.ID)
