...
(function(window) {
  ...
  function addIncidents(pageData) {
    renderIncidents(pageData['ChopsServices'], pageData['Dates'][0], pageData['Dates'][6]);
    renderIncidents(pageData['NonSLAServices'], pageData['Dates'][0], pageData['Dates'][6]);
  }
  ...
  function renderIncidents(services, firstDate, lastDate) {
	...
	if (incident['Open']) {
	  ...
	  statusCell.appendChild(getIncidentImg(incident['Severity'], IconType.CIRCLE));
	} else {
  ...
  function addIncident(incident, serviceName, firstDate, lastDate) {
    ...
    if (!startDateCell) {
      ...
      startDateCell.appendChild(getIncidentImg(incident['Severity'], IconType.CENTER));
    } else {
      startDateCell.appendChild(getIncidentImg(incident['Severity'], IconType.LEFT));
    }
    var endDateCell = getDateCell(incident['EndTime'], serviceName);
    if (!endDateCell) {
      endDateCell = getDateCell(lastDate, serviceName);
      endDateCell.appendChild(getIncidentImg(incident['Severity'], IconType.CENTER));
    } else {
      endDateCell.appendChild(getIncidentImg(incident['Severity'], IconType.RIGHT));
    }
