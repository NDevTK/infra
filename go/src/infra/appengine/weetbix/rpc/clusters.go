...
func (c *clustersServer) QueryClusterFailures(ctx context.Context, req *pb.QueryClusterFailuresRequest) (*pb.QueryClusterFailuresResponse, error) {
	...
	for _, f := range failures {
		response.Failures = append(response.Failures, createClusterFailureGroupPB(f))
	}

	return response, nil
}

func createClusterFailureGroupPB(f *analysis.ClusterFailure) *pb.ClusterFailureGroup {
	var exonerations []*pb.ClusterFailureGroup_Exoneration
	for _, ex := range f.Exonerations {
		exonerations = append(exonerations, &pb.ClusterFailureGroup_Exoneration{
	...
	var presubmitRun *pb.ClusterFailureGroup_PresubmitRun
