...
//
// eg: mergeSubClusters([[{"a":1}, {"b":2}], [{"a": 3}]]) => [{"a", [1, 3]}, {"b": [2, 0]}]
const mergeSubClusters = (subClusters: Array<SubCluster[]>): MergedSubClusters[] => {
    ...
    for (let i = 0; i < subClusters.length; i++) {
        ...
        for (const entry of clusters) {
            ...
            if (!values) {
                values = subClusters.map(_ => 0);
                lookup[entry.value] = values;
            }
            values[i] = entry.numFails;
        }
    }
    const merged: MergedSubClusters[] = [];
    for (const name in lookup) {
        merged.push({ name, values: lookup[name] })
    }
    // sort descending by first subcluster, then second, and so on.
    merged.sort((a, b) => {
