...
func decodeClusters(cc *cpb.ChunkClusters) ([][]*clustering.ClusterRef, error) {
	if cc == nil {
	...
	results := make([][]*clustering.ClusterRef, len(cc.ResultClusters))
	for i, rc := range cc.ResultClusters {
		clusters := make([]*clustering.ClusterRef, len(rc.ClusterRef))
		for j, ref := range rc.ClusterRef {
			...
			clusters[j] = &clustering.ClusterRef{
...
func encodeClusters(clusterRefs [][]*clustering.ClusterRef) *cpb.ChunkClusters {
	rb := newRefBuilder()
	resultClusters := make([]*cpb.TestResultClusters, len(clusterRefs))
	for i, refs := range clusterRefs {
		clusters := &cpb.TestResultClusters{}
		clusters.ClusterRef = make([]int64, len(refs))
		for j, r := range refs {
			clusters.ClusterRef[j] = rb.ReferenceCluster(r)
		}
...
func (rb *refBuilder) ReferenceCluster(ref *clustering.ClusterRef) int64 {
	refKey := ref.Key()
