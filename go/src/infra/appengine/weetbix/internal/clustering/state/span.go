...
func Create(ctx context.Context, e *Entry) error {
	switch {
	case !clustering.ProjectRe.MatchString(e.Project):
...
func Read(ctx context.Context, project, chunkID string) (*Entry, error) {
	columns := []string{"Project", "ChunkId", "PartitionTime", "AlgorithmsVersion", "RuleVersion", "Clusters"}
	...
	if err := b.FromSpanner(row, &result.Project, &result.ChunkID, &result.PartitionTime, &result.AlgorithmsVersion, &result.RuleVersion, clusters); err != nil {
		return nil, err
	}
...
func validateClusters(clusters [][]*clustering.ClusterRef) error {
	if len(clusters) == 0 {
