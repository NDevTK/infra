...
func TestSpanner(t *testing.T) {
	Convey(`With Spanner Test Database`, t, func() {
		...
		Convey(`ReadNext`, func() {
			...
			// Reads first page.
			rows, err := ReadNext(span.Single(ctx), testProject, readOpts)
			So(err, ShouldBeNil)
		...
		Convey(`EstimateRows`, func() {
			Convey(`Less than 100 rows`, func() {
				est, err := EstimateRows(span.Single(ctx), testProject)
				So(err, ShouldBeNil)
				So(est, ShouldBeLessThan, 100)
			})
			Convey(`At least 100 rows`, func() {
				var entries []*Entry
				for i := 0; i < 200; i++ {
				...
				count, err := EstimateRows(span.Single(ctx), testProject)
				So(err, ShouldBeNil)
