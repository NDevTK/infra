...
func (w *Worker) Do(ctx context.Context, task *taskspb.ReclusterChunks) error {
	...
	for clock.Now(ctx).Before(attemptTime) {
		...
		err := retry.Retry(ctx, transient.Only(retry.Default), func() error {
			var err error
			done, err = tctx.tryRecluster(ctx)
			return err
		}, nil)
		if err != nil {
			return err
...
// as it succeeds. It returns 'true' if the reclustering task is complete.
func (t *taskContext) tryRecluster(ctx context.Context) (done bool, err error) {
	const batchSize = 10
	...
	if len(entries) == 0 {
		err = t.pt.ReportProgress(ctx, 1000)
		if err != nil {
	...
	for _, entry := range entries {
		...
		err = t.pt.ReportProgress(ctx, progress)
		if err != nil {
...
// using the next ChunkID to process as a reference. Progress is reported as
// a proportion of the keyspace, as a value between 0 and 1000.
// 1000 is never returned by this method as the value passed is the nextChunkID,
// not the last completed chunk ID.
