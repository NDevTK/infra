...
import (
	...
	"infra/appengine/weetbix/common"
...
func TestComputeTestVariantStatusFromVerdicts(t *testing.T) {
	Convey(`ComputeTestVariantStatusFromVerdicts`, t, func() {
		...
		Convey(`still_flaky`, func() {
			...
			ms := []*spanner.Mutation{
				insert.Verdict(realm, tID, vh, "build-0", common.VerdictStatus_EXPECTED, clock.Now(ctx).UTC().Add(-time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-1", common.VerdictStatus_VERDICT_FLAKY, clock.Now(ctx).UTC().Add(-2*time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-2", common.VerdictStatus_VERDICT_FLAKY, clock.Now(ctx).UTC().Add(-3*time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-3", common.VerdictStatus_UNEXPECTED, clock.Now(ctx).UTC().Add(-4*time.Hour), nil),
			}
			testutil.MustApply(ctx, ms...)
		...
		Convey(`no_new_results`, func() {
			...
			ms := []*spanner.Mutation{
				insert.Verdict(realm, tID, vh, "build-0", common.VerdictStatus_EXPECTED, clock.Now(ctx).UTC().Add(-25*time.Hour), nil),
			}
			testutil.MustApply(ctx, ms...)
		...
		Convey(`consistently_unexpected`, func() {
			...
			ms := []*spanner.Mutation{
				insert.Verdict(realm, tID, vh, "build-0", common.VerdictStatus_VERDICT_FLAKY, clock.Now(ctx).UTC().Add(-26*time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-1", common.VerdictStatus_UNEXPECTED, clock.Now(ctx).UTC().Add(-time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-2", common.VerdictStatus_UNEXPECTED, clock.Now(ctx).UTC().Add(-2*time.Hour), nil),
			}
			testutil.MustApply(ctx, ms...)
		...
		Convey(`consistently_expected`, func() {
			...
			ms := []*spanner.Mutation{
				insert.Verdict(realm, tID, vh, "build-0", common.VerdictStatus_EXPECTED, clock.Now(ctx).UTC().Add(-time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-1", common.VerdictStatus_EXPECTED, clock.Now(ctx).UTC().Add(-2*time.Hour), nil),
			}
			testutil.MustApply(ctx, ms...)
		...
		Convey(`has_unexpected_results`, func() {
			...
			ms := []*spanner.Mutation{
				insert.Verdict(realm, tID, vh, "build-0", common.VerdictStatus_EXPECTED, clock.Now(ctx).UTC().Add(-time.Hour), nil),
				insert.Verdict(realm, tID, vh, "build-1", common.VerdictStatus_UNEXPECTED, clock.Now(ctx).UTC().Add(-2*time.Hour), nil),
			}
			testutil.MustApply(ctx, ms...)
