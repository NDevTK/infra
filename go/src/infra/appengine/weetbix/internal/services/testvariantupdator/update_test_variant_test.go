...
func TestUpdateTestVariantStatus(t *testing.T) {
	Convey(`updateTestVariant`, t, func() {
		...
		test := func(tID string, pStatuses []pb.AnalyzedTestVariantStatus, pUpdateTimes []time.Time, i int) {
			statusInfo, err := analyzedtestvariants.ReadStatus(span.Single(ctx), spanner.Key{realm, tID, vh})
			So(err, ShouldBeNil)
			statusUpdateTime := statusInfo.StatusUpdateTime
			...
			statusInfo, err = analyzedtestvariants.ReadStatus(span.Single(ctx), spanner.Key{realm, tID, vh})
			So(err, ShouldBeNil)
			So(statusInfo.Status, ShouldEqual, pb.AnalyzedTestVariantStatus_FLAKY)
			So(statusInfo.PreviousStatuses, ShouldResemble, append([]pb.AnalyzedTestVariantStatus{pb.AnalyzedTestVariantStatus_CONSISTENTLY_EXPECTED}, pStatuses...))
			So(statusInfo.PreviousStatusUpdateTimes, ShouldResemble, append([]time.Time{statusUpdateTime}, pUpdateTimes...))

			nextTask := skdr.Tasks().Payloads()[i].(*taskspb.UpdateTestVariant)
			So(pbutil.MustTimestamp(nextTask.EnqueueTime), ShouldEqual, statusInfo.NextEnqueueTime.Time)
		}
