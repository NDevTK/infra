...
func TestUpdateTestVariantStatus(t *testing.T) {
	Convey(`updateTestVariant`, t, func() {
		...
		ms := []*spanner.Mutation{
			...
			insert.Verdict(realm, tID1, vh, "build-1", common.VerdictStatus_VERDICT_FLAKY, now.Add(-2*time.Hour), nil),
			insert.Verdict(realm, tID2, vh, "build-1", common.VerdictStatus_VERDICT_FLAKY, now.Add(-2*time.Hour), nil),
		}
		testutil.MustApply(ctx, ms...)
