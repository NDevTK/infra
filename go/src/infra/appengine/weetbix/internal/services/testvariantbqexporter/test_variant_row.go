...
// based on the previous/current statuses and status update time.
...
func (b *BQExporter) timeRanges(previousStatus, currentStatus pb.AnalyzedTestVariantStatus, statusUpdateTime spanner.NullTime) map[pb.AnalyzedTestVariantStatus]*pb.TimeRange {
	...
	if sut.Before(earliest) {
		// During the entire b.options.TimeRange the test variant's status has
		// not changed. Return the original time range as it is.
		return map[pb.AnalyzedTestVariantStatus]*pb.TimeRange{currentStatus: b.options.TimeRange}
	}
	...
	trAfter := &pb.TimeRange{
		Earliest: sutP,
		Latest:   b.options.TimeRange.Latest,
	}
