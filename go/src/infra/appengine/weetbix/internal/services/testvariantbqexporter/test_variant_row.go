...
func (b *BQExporter) populateQueryParameters() (inputs, params map[string]interface{}, err error) {
	...
	case *pb.VariantPredicate_Contains:
		if len(p.Contains.Def) > 0 {
			params["VariantContains"] = true
			params["variant"] = pbutil.VariantToStrings(p.Contains)
		}
	case nil:
		// No filter.
	default:
...
func (b *BQExporter) populateTimeRange(tv *bqpb.TestVariantRow, statusUpdateTime spanner.NullTime) {
	...
	earliest, _ := pbutil.AsTime(b.BqExport.TimeRange.Earliest)
	sut := statusUpdateTime.Time
	sutP := pbutil.MustTimestampProto(sut)

	if sut.After(earliest) {
		// The test variant changed its status after the earliest.
		// Use the status update time instead of the earliest.
		tv.TimeRange.Earliest = sutP
	}
