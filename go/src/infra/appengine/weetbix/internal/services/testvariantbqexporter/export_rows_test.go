...
func (i *mockPassInserter) Put(ctx context.Context, src []*bq.Row) error {
	i.mu.Lock()
...
func (i *mockFailInserter) Put(ctx context.Context, src []*bq.Row) error {
...
func TestQueryTestVariantsToExport(t *testing.T) {
	Convey(`queryTestVariantsToExport`, t, func() {
		...
		ms := []*spanner.Mutation{
			...
			insert.Verdict(realm, tID5, vh, "build-1", pb.VerdictStatus_VERDICT_FLAKY, twoAndHalfHAgo, map[string]interface{}{
				"IngestionTime":         now.Add(-45 * time.Minute),
				"UnexpectedResultCount": 1,
				"TotalResultCount":      2,
			}),
			insert.Verdict(realm, tID5, vh, "build-2", pb.VerdictStatus_EXPECTED, oneAndHalfHAgo, map[string]interface{}{
				"IngestionTime":         halfHAgo,
				"UnexpectedResultCount": 0,
				"TotalResultCount":      1,
			}),
			...
			expRows := []*bqpb.TestVariantRow{
				{
					...
					TestMetadata: tmd,
					TimeRange: &pb.TimeRange{
						Earliest: b.TimeRange.Earliest,
						Latest:   b.TimeRange.Latest,
					},
				...
				},
				{
					...
					PartitionTime: b.TimeRange.Latest,
				},
				{
