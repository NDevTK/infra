...
func TestPurge(t *testing.T) {
	Convey(`TestPurge`, t, func() {
		...
		ms := []*spanner.Mutation{
			...
			insert.Verdict(realm, tID1, vh, "build-0", common.VerdictStatus_EXPECTED, now.Add(-time.Hour), nil),
			insert.Verdict(realm, tID4, vh, "build-1", common.VerdictStatus_VERDICT_FLAKY, now.Add(-5*30*24*time.Hour), nil),
			insert.Verdict(realm, tID4, vh, "build-2", common.VerdictStatus_EXPECTED, now.Add(-2*30*24*time.Hour), nil),
			insert.Verdict(realm, tID5, vh, "build-1", common.VerdictStatus_EXPECTED, now.Add(-2*30*24*time.Hour), nil),
			insert.Verdict(realm, tID5, vh, "build-2", common.VerdictStatus_VERDICT_FLAKY, now.Add(-5*24*time.Hour), nil),
		}
