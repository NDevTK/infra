...
type ClusteredFailuresClient interface {
	...
	Insert(ctx context.Context, luciProject string, tableCfg *config.BigQueryTable, rows []*bqpb.ClusteredFailure) error
}
...
func (r *ClusteringHandler) ClustersUpdated(ctx context.Context, updates *clustering.Update, commitTime time.Time) error {
	...
	projectCfg, ok := projects[updates.Project]
	if !ok {
		return fmt.Errorf("config for project %q does not exist", updates.Project)
	}
	tableCfg := projectCfg.ClusteredFailuresTable

	rowUpdates := prepareInserts(updates, commitTime)
	return r.clusteredFailures.Insert(ctx, updates.Project, tableCfg, rowUpdates)
}

// prepareInserts prepares entries into the BigQuery clustered failures table
// in response to a reclustering. For efficiency, only the updated rows are
// returned.
func prepareInserts(updates *clustering.Update, commitTime time.Time) []*bqpb.ClusteredFailure {
...
func entryFromUpdate(project, chunkID string, cluster *clustering.ClusterID, failure *cpb.Failure, included, includedWithHighPriority bool, commitTime time.Time) *bqpb.ClusteredFailure {
	...
	entry := &bqpb.ClusteredFailure{
		...
		RootInvocationResultIndex: failure.RootInvocationResultIndex,
