...
func (c *Client) GetMetrics(ctx context.Context, dataSource string, period api.Period, dates, metrics []string) ([]*api.Section, error) {
	...
	case api.Period_DAY:
		configPeriod = Day
	default:
		return nil, appstatus.Errorf(codes.InvalidArgument, "period not recognized")
	}

	if _, exists := c.Config.Sources[dataSource].Queries[configPeriod]; !exists {
		return nil, appstatus.Errorf(codes.InvalidArgument, "period %q not available for data source %q", configPeriod, dataSource)
	}
	...
	q := c.Client.Query(c.Config.Sources[dataSource].Queries[configPeriod])
	...
	for {
		...
		if !ok {
			metric = &api.Metric{
				...
				Sections: make(map[string]*api.DataSet),
			}
