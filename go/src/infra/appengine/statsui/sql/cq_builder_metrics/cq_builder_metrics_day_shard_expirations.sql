-- This query updates the 'Shard Expirations' metrics in the cq_builder_metrics_day
-- table. It uses an interval of the last 2 days so that there is some
-- redundancy if the job fails This query is meant to be almost identical to
-- the one in cq_builder_metrics_weed_shard_expirations.sql.

-- The lines below are used by the deploy tool.
--name: Populate cq_builder_metrics_day Shard Expirations
