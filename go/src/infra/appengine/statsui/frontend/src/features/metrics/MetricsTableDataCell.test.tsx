...
describe('when calculating colors', () => {
  ...
  it('should return color for negative percentage delta', () => {
    const metric: MetricOption = Object.assign({}, base);
    metric.color = {
    ...
    expect(
      testables.calculateColor(metric, { value: 10, previous: { value: 20 } })
    ).toStrictEqual('#fff');
  });
  it('should return color for empty value', () => {
    ...
    expect(testables.calculateColor(metric, { value: 10 })).toStrictEqual(
      '#fff'
    );
  });
});
