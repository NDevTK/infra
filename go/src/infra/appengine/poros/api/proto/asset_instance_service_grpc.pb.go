...
func (c *assetInstanceClient) FetchLogs(ctx context.Context, in *FetchLogsRequest, opts ...grpc.CallOption) (*FetchLogsResponse, error) {
	out := new(FetchLogsResponse)
	err := c.cc.Invoke(ctx, "/poros.AssetInstance/FetchLogs", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _AssetInstance_FetchLogs_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(FetchLogsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AssetInstanceServer).FetchLogs(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/poros.AssetInstance/FetchLogs",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AssetInstanceServer).FetchLogs(ctx, req.(*FetchLogsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
