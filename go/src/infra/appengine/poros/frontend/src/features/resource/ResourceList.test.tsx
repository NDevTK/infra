...
test('Should have all the required columns', () => {
  ...
  expect(within(screen.getAllByRole('row')[0]).getAllByRole('columnheader')).toHaveLength(8);
});
...
test('Should have just header row when resources are empty', () => {
  ...
  expect(within(screen.getAllByRole('row')[0]).queryAllByRole('cell').length).toBe(0);
});
