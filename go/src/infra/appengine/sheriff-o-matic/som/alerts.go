...
func getRestartingMasters(c context.Context, treeName string) (map[string]masterState, error) {
	...
	if !ok {
		return nil, fmt.Errorf("Unrecognized tree name: %v", treeName)
	}

	logging.Infof(c, "treeCfg: %+v", treeCfg)
	now := time.Now().UTC()

	ret := map[string]masterState{}
	for masterLoc := range treeCfg.Masters {
		logging.Infof(c, "checking %v master %v", treeName, masterLoc.Name())
		for _, ds := range ms.MasterStates["master."+masterLoc.Name()] {
			...
			// TODO: make this warning window confgurable. This logic will include a
