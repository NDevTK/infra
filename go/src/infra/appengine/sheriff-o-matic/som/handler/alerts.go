...
func FlushOldAlertsHandler(ctx *router.Context) {
	...
	numDeleted, err := flushOldAlerts(c, resolvedAlertExpiration)
	if err != nil {
		errStatus(c, w, http.StatusInternalServerError, err.Error())
		return
	}
...
func flushOldAlerts(c context.Context, duration time.Duration) (int, error) {
	q := datastore.NewQuery("AlertJSONNonGrouping").Eq("Resolved", true)
	q = q.Lt("ResolvedDate", clock.Get(c).Now().Add(-duration))
	q = q.KeysOnly(true)
