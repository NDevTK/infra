      ...
      body: JSON.stringify(formData),
        ...
        .then((resp) => {
          if (resp.ok) {
            ...
            window.console.error('Non-OK response for updating expectation',
                                 resp);
          }
        })
        .catch((error) => {
            ...
            resp.json().then((data) => {
              if (data.ChangeID != '') {
                ...
                this._pollingTask = this.async(
                    this._pollCLStatus.bind(this, attempt), delay * 1000);
              } else {
                this._statusMessage =
                    'Too many CL status fetch attempts. Giving up.';
              }
            });
          } else {
