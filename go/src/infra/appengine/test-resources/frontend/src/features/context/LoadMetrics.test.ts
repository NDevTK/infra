...
import { MetricType, Period, SortType, TestDateMetricData, TestMetricsArray } from '../../api/resources';
import { computeDates, dataReducer } from './LoadMetrics';

function metricsMap(metrics: {[date: string]: [MetricType, number][]}): Map<string, TestMetricsArray> {
  const ret = new Map<string, TestMetricsArray>();
...
describe('Merge TestMetrics', () => {
  it('populate tests with a single variant correctly', () => {
    ...
    expect(merged[0].metrics.get('2012-01-02')?.get(MetricType.NUM_RUNS)).toEqual(1);
    expect(merged[0].metrics.get('2012-01-02')?.get(MetricType.NUM_FAILURES)).toEqual(2);
    ...
    expect(v.metrics.get('2012-01-02')?.get(MetricType.NUM_FAILURES)).toEqual(2);
  });
  ...
  it('populate tests with a multiple dates correctly', () => {
    ...
    expect(merged[0].metrics.get('2012-01-02')?.get(MetricType.NUM_RUNS)).toEqual(1);
    expect(merged[0].metrics.get('2012-01-02')?.get(MetricType.NUM_FAILURES)).toEqual(2);
    expect(merged[0].metrics.get('2012-01-03')?.get(MetricType.NUM_RUNS)).toEqual(3);
    expect(merged[0].metrics.get('2012-01-03')?.get(MetricType.NUM_FAILURES)).toEqual(4);
  });
});
