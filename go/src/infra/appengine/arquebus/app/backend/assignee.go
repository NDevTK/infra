...
func fetchOncallFromRotationProxy(c context.Context, rotation string, oc *oncallShift) error {
	...
	if shifts := resp.GetShifts(); shifts != nil {
...
func findShift(c context.Context, task *model.Task, rotation string, bool useRotationProxy) (*oncallShift, error) {
	var oc oncallShift
	...
	if useRotationProxy {
		fetchOncallFromRotationProxy(c, rotation, &oc)
	} else {
		fetchOncallFromRotaNg(c, rotation, &oc)
	}
...
func findOncallers(c context.Context, task *model.Task, oncall *config.Oncall, bool useRotationProxy) ([]*monorail.UserRef, error) {
