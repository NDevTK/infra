...
var timeWindowToDuration = map[string]time.Duration{
	"s": time.Second,
	"m": time.Minute,
	"h": time.Hour,
	"d": 24 * time.Hour,
}
var timeWindowToStr = map[string]string{
	"s": "second(s)",
	"m": "minute(s)",
	"h": "hour(s)",
	"d": "day(s)",
...
// is not a clean revert. When the string is an empty string, it means the CL
// is a clean revert.
func reviewCleanRevert(ctx context.Context, cfg *config.Config, gc gerrit.Client, t *taskspb.ChangeReviewTask) (string, error) {
	...
	// Check whether the change is in a valid time window.
	...
	if !ok {
		return fmt.Sprintf("The change is not in the valid time window. Rubber Stamper is only allowed to review reverts within %s %s.", tw[:len(tw)-1], timeWindowToStr[tw[len(tw)-1:]]), nil
	}
