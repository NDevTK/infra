...
func CommitScanner(rc *router.Context) {
	...
	// Defer persisting c.LastKnownCommit and c.LastRelevantCommit so that
	// partial progress is saved even in the case of unexpected failure.
	defer func() {
		...
		for _, rule := range repoRules {
			...
			case commit.Author.Email, commit.Committer.Email:
				if c.LastRelevantCommit != commit.Commit {
					...
					// If one rule matches, that's enough,
