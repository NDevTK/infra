...
func getSatlabStableVersion(ctx context.Context, ic inventoryClient, buildTarget string, model string, hostname string) (*fleet.GetStableVersionResponse, bool, error) {
	logging.Infof(ctx, "using satlab flow %q %q %q", buildTarget, model, hostname)

	hostnameID := satlab.MakeSatlabStableVersionID(hostname, "", "")
	if hostnameID != "" {
	...
	if buildTarget == "" || model == "" {
		logging.Infof(ctx, "looking up inventory info for DUT h=%q b=%q m=%q in satlab flow", hostname, buildTarget, model)
		dut, err := getDUT(ctx, ic, hostname)
		if err != nil {
			return nil, false, errors.Annotate(err, "failed to get DUT %q", hostname).Err()
		}
	...
	boardModelID := satlab.MakeSatlabStableVersionID("", buildTarget, model)
	if boardModelID != "" {
...
func getStableVersionImpl(ctx context.Context, ic inventoryClient, buildTarget string, model string, hostname string, satlabInformationalQuery bool) (*fleet.GetStableVersionResponse, error) {
	...
	if heuristics.LooksLikeSatlabDevice(hostname) || satlabInformationalQuery {
		wantSatlab = true
