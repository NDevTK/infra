...
// getIdleDuration queries swarming for the duration since last task on the bot with bot id bid.
func getIdleDuration(c context.Context, sc clients.SwarmingClient, bid string) (*duration.Duration, error) {
	trs, err := sc.ListSortedRecentTasksForBot(c, bid, 1)
	if err != nil {
		return nil, errors.Annotate(err, "failed to list recent tasks for bot %s", bid).Err()
	}
	if len(trs) == 0 {
		return nil, nil
	}
	return timeSinceTask(trs[0])
}

func timeSinceTask(tr *swarming.SwarmingRpcsTaskResult) (*duration.Duration, error) {
	switch tr.State {
	case "RUNNING":
