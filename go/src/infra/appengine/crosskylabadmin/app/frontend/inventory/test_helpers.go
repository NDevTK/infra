...
func (g *fakeGitilesClient) addArchive(ic *config.Inventory, labData []byte, inventoryDate []byte) error {
	...
	if err := tw.WriteHeader(&tar.Header{
		...
		Mode: 0777,
		Size: int64(len(inventoryDate)),
	}); err != nil {
		return err
	}
	if _, err := tw.Write(inventoryDate); err != nil {
		return err
	}
