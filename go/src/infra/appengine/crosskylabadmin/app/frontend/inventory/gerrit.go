...
func commitLabInventoryStr(ctx context.Context, client gerrit.GerritClient, lab string) (url string, err error) {
	...
	fileName := inventoryConfig.LabDataPath
	return commitStringToFile(ctx, client, lab, fileName)
}
...
func commitInfraInventoryStr(ctx context.Context, client gerrit.GerritClient, infra string) (url string, err error) {
	inventoryConfig := config.Get(ctx).Inventory
	fileName := inventoryConfig.InfrastructureDataPath
	return commitStringToFile(ctx, client, infra, fileName)
}

func commitStringToFile(ctx context.Context, client gerrit.GerritClient, contents string, fileName string) (url string, err error) {
	inventoryConfig := config.Get(ctx).Inventory
