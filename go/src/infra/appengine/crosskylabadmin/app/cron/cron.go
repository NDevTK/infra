...
func InstallHandlers(r *router.Router, mwBase router.MiddlewareChain) {
	...
	r.GET("/internal/cron/push-labstations-for-repair", mwCron, logAndSetHTTPErr(pushLabstationsForRepairCronHandler))
...
func pushLabstationsForRepairCronHandler(c *router.Context) (err error) {
	defer func() {
		pushLabstationsForRepairCronHandlerTick.Add(c.Context, 1, err == nil)
	}()
	...
	tsi := frontend.TrackerServerImpl{}
	if _, err := tsi.PushLabstationsForRepair(c.Context, &fleet.PushLabstationsForRepairRequest{}); err != nil {
		return err
	}
	logging.Infof(c.Context, "Successfully finished")
	return nil
