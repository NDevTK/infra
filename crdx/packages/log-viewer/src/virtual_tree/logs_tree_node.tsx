...
/**
 * Defines the color props used in the tree node, it will use the default
 * if anu is not provided.
 */
export interface TreeNodeColors {
  ...
  unsupportedBackgroundColor?: string;
}
...
export const LogsTreeNode = memo(function LogsTreeNode({
  ...
  const getNodeBackgroundColor = (node: TreeData<ObjectNode>) => {
    if (node.data.deeplinked)
      return colors?.deepLinkBackgroundColor ?? teal[100];
  ...
  const getNodeText = (node: ObjectNode): JSX.Element => {
    ...
    if (node.size === 0) {
      return (
        <span
          style={{
            color: colors?.unsupportedBackgroundColor ?? grey[600],
          }}
        >
          {nodeInnerText}
        </span>
