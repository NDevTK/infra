...
@mock.patch.object(os, 'open', _noop_handler)
@mock.patch.object(os, 'close', _noop_os_close)
@mock.patch.object(os, 'fstat', _noop_fstat)
class TestFlock(unittest.TestCase):
  ...
  def _set_stat_status(self, success=True, matching=True):
    ...
    return mock.patch.object(os, 'stat', _stat_handler)
  ...
  def _set_unlink_status(self, success=True):
    ...
    return mock.patch.object(os, 'unlink', _unlink_handler)

  #### Tests.

  def testGetLock(self):
    with self._set_lock_status(), self._set_stat_status(
  ...
  def testDontGetLock(self):
    with self._set_lock_status(
        success=False), self._set_stat_status(), self._set_unlink_status():
  ...
  def testFileDeletedAfterLockAcquired(self):
    ...
    with self._set_lock_status(), self._set_stat_status(
        success=False), self._set_unlink_status():
      with self.assertRaises(daemon.LockAlreadyLocked):
        with daemon.flock('bogus'):
          # Should never reach this.
          # pylint: disable=redundant-unittest-assert
  ...
  def testLockfileRecreated(self):
    ...
    with self._set_lock_status(), self._set_stat_status(
        matching=False), self._set_unlink_status():
      with self.assertRaises(daemon.LockAlreadyLocked):
        with daemon.flock('bogus'):
          # Should never reach this.
          # pylint: disable=redundant-unittest-assert
  ...
  def testDeleteWhenDone(self):
    ...
    with mock.patch.object(os, 'unlink', _mock_unlink):
      with self._set_lock_status(), self._set_stat_status():
        with self._assert_reached() as reached:
          with daemon.flock('bogus'):
            reached['yup'] = True
