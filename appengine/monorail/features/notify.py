...
class NotifyApprovalChangeTask(notify_helpers.NotifyTaskBase):
  ...
  def HandleRequest(self, mr):
    ...
    field_user_ids = set()
    fvs = [fv for fv in issue.field_values if not fv.field_id or
           fv.field_id is approval_value.approval_id]
    for fd in config.field_defs:
      field_user_ids.update(notify_reasons.ComputeNamedUserIDsToNotify(fvs, fd))
    users_by_id = framework_views.MakeAllUserViews(
