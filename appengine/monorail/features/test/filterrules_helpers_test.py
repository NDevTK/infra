...
class RecomputeAllDerivedFieldsTest(unittest.TestCase):
  ...
  def testRecomputeDerivedFields_SomeIssues(self, get_client_mock):
    ...
    highest_id = self.services.issue.GetHighestLocalID(self.cnxn, self.project.project_id)
  ...
  def testRecomputeDerivedFields_LotsOfIssues(self, get_client_mock):
    ...
    ((_parent, called_task), _kwargs) = get_client_mock().create_task.call_args_list[0]
    relative_uri = called_task.get('app_engine_http_request').get('relative_uri')
    parse_result = urlparse.urlparse(relative_uri)
    self.assertEqual(parse_result.path, urls.RECOMPUTE_DERIVED_FIELDS_TASK + '.do')
    ...
    self.assertEqual(params['lower_bound'], str(12345 // self.BLOCK * self.BLOCK + 1))
  ...
  @mock.patch('features.filterrules_helpers.ApplyGivenRules', return_value=(True, {}))
  def testRecomputeAllDerivedFieldsNow(self, apply_mock):
    ...
    for test_issue in test_issues:
      apply_mock.assert_any_call(self.cnxn, self.services, test_issue, self.config, [], [])
