...
class HotlistDetailsTest(unittest.TestCase):
  ...
  def testGatherPageData(self):
    ...
    # editor is viewing, so read_only
    self.assertEqual(ezt.boolean(True), page_data['read_only'])

    # owner is veiwing, so read_only is False
    self.mr.auth.effective_ids = [111L]
    page_data = self.servlet.GatherPageData(self.mr)
    self.assertEqual(ezt.boolean(False), page_data['read_only'])
