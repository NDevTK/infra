...
def _AssertNoConflictingDeltas(issue_delta_pairs, refs_dict, err_agg):
  ...
  for issue, delta in issue_delta_pairs:
    ...
    # Get intersection of issues that are both removed and added.
    conflict_iids = set(delta.blocking_remove) & set(added_iids)
    if conflict_iids:
      conflict_refs.extend([refs_dict[iid] for iid in conflict_iids])

    for conflict_iid in delta.blocked_on_remove:
      if issue.issue_id in blocking_add[conflict_iid]:
        conflict_refs.append(refs_dict[conflict_iid])

    if conflict_refs:
      refs_str = ', '.join(conflict_refs)
