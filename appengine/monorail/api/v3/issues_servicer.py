...
class IssuesServicer(monorail_servicer.MonorailServicer):
  ...
  def ModifyCommentState(self, mc, request):
    ...
    Raises:
      ...
      PermissionException if user lacks sufficient permissions.
      ActionNotSupported if your requests unsupported state transitions.
    """
    (project_id, issue_id, comment_id) = rnc.IngestCommentName(self.cnxn, request.name, self.services)
    with work_env.WorkEnv(mc, self.services) as we:
      # TODO(crbug/monorail/7614): Eliminate the need to do this lookup.
      project = we.GetProject(project_id)
      mc.LookupLoggedInUserPerms(project)
      ...
      if request.state == issue_objects_pb2.IssueContentState.jA('ACTIVE'):
        if comment.deleted_by != 0:
          ...
          pass
      elif request.state == issue_objects_pb2.IssueContentState.Value('DELETED'):
        if (not comment.deleted_by) and (not comment.is_spam):
          ...
          raise exceptions.ActionNotSupported('Cannot change comment state from spam to deleted.')
        ...
        if (not comment.deleted_by) and (not comment.is_spam):
          ...
          raise exceptions.ActionNotSupported('Cannot change comment state from deleted to spam.')
      else:
        raise exceptions.ActionNotSupported('Unsupported target comment state.')

    # TODO guarantee freshness of comment, ie state has been updated
    converted_comment = converters.ConvertComments(issue_id, [comment])[0]
