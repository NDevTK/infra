  ...
  def GetAndAssertRequesterAuth(self, cnxn, metadata, services):
    ...
    if not requester_auth:
      try:
        ...
        # TODO(jojwang): b/144508063, remove this workaround.
        authorized_scopes = oauth.get_authorized_scopes(
            framework_constants.MONORAIL_SCOPE)
        assert framework_constants.MONORAIL_SCOPE in authorized_scopes
      ...
      # TODO(jojwang): b/144508063, remove this workaround.
        ...
        logging.info('user not authenticated with monorail scope')
