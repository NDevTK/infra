...
class Converter(object):
  ...
  def _FillIssueFromTemplate(self, template, project_id):
    ...
    labels = self.ConvertTemplateLabels(template.labels, project_id)
    ...
    enum_field_values = self.ConvertEnumFieldValues(template.labels, project_id)
  ...
  def ConvertTemplateLabels(self, labels, project_id):
    ...
    non_fd_labels, _derived_labels = tbo.ExplicitAndDerivedNonMaskedLabels(
        labels, [], config)
    api_labels = []
  ...
  def ConvertEnumFieldValues(self, labels, project_id):
    # type: (Sequence[str], int) ->
    #     Sequence[api_proto.issue_objects_pb2.Issue.FieldValue]
    ...
    config = self.services.config.GetProjectConfig(self.cnxn, project_id)
    enum_names_by_lower = {
        fd.field_name.lower(): fd.field_name for fd in config.field_defs if
        fd.field_type is tracker_pb2.FieldTypes.ENUM_TYPE and not fd.is_deleted
    }

    labels_by_prefix = tbo.LabelsByPrefix(labels, enum_names_by_lower.keys())
