...
class ConverterFunctionsTest(unittest.TestCase):
  ...
  def testConvertFieldValues_IgnoresNullFieldDefs(self):
    ...
    expected_int = 111111
    fv_2 = fake.MakeFieldValue(
        field_id=self.dne_field_def_id, int_value=expected_int, derived=True)
    name_2 = rnc.ConvertFieldDefNames(
        self.cnxn, [self.field_def_2], self.project_1.project_id,
        self.services).get(self.field_def_2)
  ...
  def test_ComputeFieldValueValue_None(self):
    self.assertEqual(converters._ComputeFieldValueValue(None), None)

  def test_ComputeFieldValueValue_INT_TYPE(self):
    expected = 123158
    fv = fake.MakeFieldValue(field_id=self.field_def_2, int_value=expected)
    output = converters._ComputeFieldValueValue(fv)
    self.assertEqual(str(expected), output)

  def test_ComputeFieldValueValue_STR_TYPE(self):
    expected = 'some_string_field_value'
    fv = fake.MakeFieldValue(field_id=self.field_def_1, str_value=expected)
    output = converters._ComputeFieldValueValue(fv)
    self.assertEqual(expected, output)

  def test_ComputeFieldValueValue_USER_TYPE(self):
  ...
  def test_ComputeFieldValueValue_DATE_TYPE(self):
    expected = 1234567890
    fv = fake.MakeFieldValue(
  ...
  def test_ComputeFieldValueValue_URL_TYPE(self):
    expected = 'some URL'
