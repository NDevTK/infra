...
class ConfigServiceTest(unittest.TestCase):
  ...
  def SetUpCreateFieldDef(self, project_id):
    ...
    self.config_service.fielddef2editor_tbl.InsertRows(
        self.cnxn, config_svc.FIELDDEF2EDITOR_COLS, [(1, 111)], commit=False)
    self.cnxn.Commit()
  ...
  def SetUpUpdateFieldDef(self, field_id, new_values, admin_ids, editor_ids):
    self.config_service.fielddef_tbl.Update(
    ...
    self.config_service.fielddef2admin_tbl.InsertRows(
        self.cnxn, config_svc.FIELDDEF2ADMIN_COLS, admin_ids, commit=False)
    self.config_service.fielddef2editor_tbl.Delete(
        self.cnxn, field_id=field_id, commit=False)
    self.config_service.fielddef2editor_tbl.InsertRows(
        self.cnxn, config_svc.FIELDDEF2EDITOR_COLS, editor_ids, commit=False)
    self.cnxn.Commit()
  ...
  def testUpdateFieldDef_Normal(self):
    new_values = dict(
        ...
        docstring='new doc',
        is_restricted_field=True)
    self.SetUpUpdateFieldDef(1, new_values, [(1, 111)], [(1, 111)])

    self.mox.ReplayAll()
    new_values = new_values.copy()
    new_values['notify_on'] = 1
    self.config_service.UpdateFieldDef(
        self.cnxn, 789, 1, admin_ids=[111], editor_ids=[111], **new_values)
    self.mox.VerifyAll()
