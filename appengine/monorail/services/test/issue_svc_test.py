...
class IssueServiceTest(unittest.TestCase):
  ...
  def testInsertComment_WithIssueUpdate(self):
    ...
    self.SetUpInsertComment(7890101, amendment_rows=amendment_rows)
    self.mox.ReplayAll()
    self.services.issue.InsertComment(self.cnxn, comment, commit=True)
    self.services.issue.issueupdate_tbl.InsertRow.assert_called_with(
        self.cnxn,
        issue_svc.ISSUEUPDATE_COLS[1:], 
        amendment_rows,
        commit=False)    
    self.mox.VerifyAll()
    self.assertEqual(7890101, comment.id)
