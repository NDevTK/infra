...
class IssueServiceTest(unittest.TestCase):
  ...
  def testInsertComment_WithIssueUpdate(self):
    ...
    self.service.issue.issueupdate_tbl.InsertRows = Mock()
    self.service.issue.commentcontent_tbl.InsertRow = Mock(return_value=commentcontent_id)
    self.service.issue.comment_tbl.InsertRow = Mock(return_value=comment_id)
    self.services.issue.InsertComment(self.cnxn, comment, commit=True)
    self.assertEqual(comment_id, comment.id)
    self.service.issue.commentcontent_tbl.InsertRow.assert_called_once_with(
