...
class ChartService(object):
  ...
  def QueryIssueSnapshots(self, cnxn, services, unixtime, effective_ids,
    ...
    for shard_id in range(settings.num_logical_shards):
      ...
      print stmt_str, stmt_args

      real_stmt = 'SELECT COUNT(results.issue_id) FROM (%s) AS results' % stmt_str
      print 'FINAL STATEMENT -----------'
      print real_stmt

      # result = cnxn.Execute(real_stmt, stmt_args)
      p = framework_helpers.Promise(cnxn.Execute, real_stmt, stmt_args, shard_id=shard_id)
      promises.append(p)
