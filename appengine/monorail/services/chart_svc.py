...
class ChartService(object):
  ...
  def _QueryToWhere(self, cnxn, services, project_config, query, project):
    ...
    print '*' * 300
    print 'query_ast', query_ast
    print 'QUERY', query
    print 'LEFT_JOINS', left_joins
    print 'WHERE', where
    print 'UNSUPPORTED', unsupported
    print '*' * 300

    '''Proof of concept: this works for q=label:Performance by manually replacing strings
    left_joins[0] = (left_joins[0][0].replace('AND Issue.shard = Cond0.issue_shard ', ''), left_joins[0][1])
    left_joins[0] = (left_joins[0][0].replace('Issue2Label', 'IssueSnapshot2Label'), left_joins[0][1])
    left_joins[0] = (left_joins[0][0].replace('Issue.', 'IssueSnapshot.'), left_joins[0][1])
    left_joins[0] = (left_joins[0][0].replace('Cond0.issue_id', 'Cond0.issuesnapshot_id'), left_joins[0][1])
    '''
