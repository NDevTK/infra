...
def BuildSQLQuery(query_ast, snapshot_mode=False):
  ...
  for cond_num, cond in enumerate(conj.conds):
    cond_left_joins, cond_where, unsupported = _ProcessCond(cond_num, cond, snapshot_mode)
...
def _ProcessRelatedIDCond(cond, alias, kind, reverse_relation=False, snapshot_mode=False):
...
def _ProcessOwnerCond(cond, alias, _user_alias, snapshot_mode):
  ...
  # TODO(CL): Support IssueSnapshot.owner_id
  if snapshot_mode:
    # TODO(CL): Is eliminating derived_owner_id on Snapshots ok?
    left_joins = [(
        'User AS {alias} ON IssueSnapshot.owner_id = {alias}.user_id'.format(alias=alias),
        [])]
  else:
    left_joins = [(
...
def _ProcessOwnerIDCond(cond, _alias, _user_alias, snapshot_mode):
  ...
  if snapshot_mode:
    field_type, field_values = _GetFieldTypeAndValues(cond)
    # TODO(CL): Is it alright to not store/use derived_owner_id?
...
def _ProcessReporterCond(cond, alias, _user_alias, snapshot_mode):
  ...
  # TODO(CL): Support IssueSnapshot.reporter_id
  left_joins = [(
...
def _ProcessCcCond(cond, alias, user_alias, snapshot_mode):
  ...
  # TODO(CL): Support IssueSnapshot2Cc
...
def _ProcessStatusIDCond(cond, _alias, _user_alias, snapshot_mode):
  ...
  # TODO(CL): Um... should we have IssueSnapshot.status_id?
  if snapshot_mode:
    return [], [], [cond]
...
def _ProcessHotlistIDCond(cond, alias, _user_alias, snapshot_mode):
  ...
  if snapshot_mode:
    join_str = (
      'IssueSnapshot2Hotlist AS {alias} ON IssueSnapshot.id = {alias}.issuesnapshot_id'.format(
          alias=alias))
  else:
    join_str = (
...
def _ProcessCond(cond_num, cond, snapshot_mode):
  ...
  # TODO(CL): Support snapshot_mode (return unsearchable fields)
