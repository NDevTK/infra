...
(function(window) {
  ...
  const PROJECT_LOCALID_STR = '([ \t]*(?<projectName>\b[-a-z0-9]+[:\#])?(?<numberSign>\#?)(?<localId>\d+)\b(,?[ \t]*(and|or)?)?)'
  ...
  function ExtractCrbugProjectAndIssueIds(match, defaultProjectName='chromium') {
    ...
    const projectName = groups.projectName ? groups.projectName : defaultProjectName;
    return {projectName: projectName, localId: groups.localId};
  }

  function ExtractProjectAndIssueIds(match, defaultProjectName='chromium') {
    const issueRefRE = new RegExp(PROJECT_LOCALID_STR);
    let refMatch;
    let refs = [];
	...
	defaultProjectName = refMatch.groups.projectName.slice(0, -1);
      ...
      refs.push({projectName: defaultProjectName, localId: groups.localId});
    }
    return refs;
  }
  ...
  function ReplaceTrackerIssueRef(match, components, defaultProjectName='chromium') {
    const refsFromText = ExtractProjectAndIssueIds(match, defaultProjectName);
    let textRuns = [];
    refsFromText.forEach(({projectName, localId}) => {
      textRuns.push(ReplaceIssueRef(match[0], projectName, localId, components));
    });
    return textRuns;
  }
