...
export class MrStarButton extends connectStore(LitElement) {
  ...
  render() {
    return html`
      ...
      >
        ${this._isIssueStarred.has(issueRefToString(this.issueRef)) ? html`
  ...
  stateChanged(state) {
    ...
    this._isIssueStarred = issue.allStarredIssues(state);
  ...
  toggleStar() {
    ...
    const newIsStarred = !this._isIssueStarred.has(
