...
export class MrStarButton extends connectStore(LitElement) {
  ...
  render() {
    return html`
      ...
      >
        ${this._isStarred.get(issueRefToString(this.issueRef)) ? html`
  ...
  static get properties() {
    return {
      ...
      /**
       * Whether the issue is starred (value that maps to issueRef).
       */
      _isStarred: {type: Map},
      /**
  ...
  stateChanged(state) {
    ...
    this._isStarred = issue.isIssueStarred(state);
  ...
  toggleStar() {
    ...
    const newIsStarred = !this._isStarred.get(issueRefToString(this.issueRef));
