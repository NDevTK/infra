...
export class MrStarButton extends connectStore(LitElement) {
  ...
  render() {
    return html`
      ...
      >
        ${this._isIssueStarred.get(issueRefToString(this.issueRef)) ? html`
  ...
  static get properties() {
    return {
      ...
      /**
       * Whether the issue is starred (used for updating store).
       */
      _isStarred: {type: Object},
      /**
       * Whether the issue is starred (used for accessing easily).
       */
      _isIssueStarred: {type: Map},
      /**
  ...
  stateChanged(state) {
    ...
    this._isIssueStarred = issue.isIssueStarred(state);
  ...
  toggleStar() {
    ...
    const newIsStarred = !this._isIssueStarred.get(
    ...
    // Here we access and pass in the object version of isStarred instead
    // of using this._isStarred in order to more easily update store.

    store.dispatch(issue.star(this.issueRef, newIsStarred, this._isStarred));
  }
}
