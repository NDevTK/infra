...
class AllStatusPage(BasePage):
  ...
  # TODO (crbug.com/1121016): Require read access
  def get(self):
    ...
    if end_date:
      ...
      # |end_date|.
      beyond_end_of_range_status = Status.query(
          ).filter(Status.date < parse_date(end_date)).order('-date').get()
...
class CurrentPage(BasePage):
  ...
  def get(self):
    ...
    if out_format == 'html' and not self.read_access and not self.user:
      ...
      return self.DisplayTemplate('current.html', template_values, use_cache=True)
    else:
      return self._handle()
...
class MainPage(BasePage):
  ...
  def _handle(self, error_message='', last_message=''):
    ...
    limit = limit = int(request.args.get('limit', '25'))
  ...
  def post(self):
    ...
    current_status = get_status()
    if current_status and (last_status_key != str(current_status.key.integer_id())):
