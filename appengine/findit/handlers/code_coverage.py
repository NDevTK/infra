...
class ServeCodeCoverageData(BaseHandler):
  ...
  def _ServePerCLCoverageData(self):
    ...
    2. Percentages are usually requested much ealier than lines by the Gerrit
    ...
    logging.info('type=%d', data_type)

    if data_type not in ('lines', 'percentages'):
      return BaseHandler.CreateError(
          error_message=(
              'Invalid type: %d, must be "lines" (default) or percentages' %
              data_type),
          return_code=400,
          allowed_origin='*')
