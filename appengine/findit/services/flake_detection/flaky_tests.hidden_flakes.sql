  # To detect hidden flaky tests causing test retries in the past 1 days.
  #
  # Assumptions for the hidden flakes in this query are:
  # 1. A hidden flaky test has
...
WITH
  ...
  FROM (
    SELECT
      ...
      FROM
        UNNEST(run.actual) AS actual) AS expected_runs,
      run.actual,
      run.expected
    FROM
  ...
  WHERE
    ...
    expected_runs > 0
    AND expected_runs < ARRAY_LENGTH(actual))
...
SELECT
  ...
  flake.expected_runs as expected_runs,
