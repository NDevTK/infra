...
# 2. Failed in the (with patch) step, and failed again in the (retry with patch)
#    step in the same flaky build as defined above.
#
# Caveat:
  ...
  SELECT
    ...
    (ARRAY(
      ...
      HAVING
        ...
        # both (with patch) and (retry with patch).
        # There is no need to check (without patch) step because if a test is
        # consistently failing on ToT due to a bad commit, it would fail in
        # (without patch), and it won't run in the (retry with patch) step at all.
