...
def _CreateAuthorCoverage(postsubmit_report, builder):
  build = GetV2Build(
  ...
  for gs_metadata_dir, mimic_builder_name in zip(gs_metadata_dirs,
    ...
    for file_name in blamelist:
      ...
      for author in blamelist[file_name]:
        ...
        # and create an active CoverageReportModifier corresponding to the googler,
        ...
        if modified_line_coverage:
          ...
          total_entities += 1
          author_file_coverage_entities.append(entity)
          author_file_coverage_entities, total_entities = _FlushEntities(
              author_file_coverage_entities, total_entities)
          aggregator.consume_file_coverage(modified_line_coverage)

        # Create remaining FileCoverageData entities
        _FlushEntities(author_file_coverage_entities, total, last=True)

        modified_directory_coverage = aggregator.produce_summary_coverage()
        _ExportDirSummaryCoverage(modified_directory_coverage,
                                  postsubmit_report, modifier.id)
