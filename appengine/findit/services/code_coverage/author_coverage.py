...
def _CreateAuthorCoverage(postsubmit_report):
  ...
  for gs_metadata_dir, mimic_builder_name in zip(gs_metadata_dirs,
    ...
    total_entities = 0
    modified_file_coverage_entities = defaultdict(list)
    ...
    for file_name in blamelist:
      ...
      for author in blamelist[file_name]:
        ...
        modifier = CoverageReportModifier.InsertModifierForAuthorIfNeeded(
            postsubmit_report.gitiles_commit.server_host,
            postsubmit_report.gitiles_commit.project, googler)
        ...
        if modified_line_coverage:
          # and create corresponding FileCoverageData entity and process it
          # towards summary coverage.
          ...
          total_entities += 1
          modified_file_coverage_entities[modifier.key.id()].append(entity)
          aggregators[modifier.key.id()].consume_file_coverage(
    ...
    for modifier_id, aggregator in aggregators.items():
