...
class PassRateUtilTest(WaterfallTestCase):
  ...
  def testCalculateNewPassRate(self):
    self.assertEqual(
        0.75, pass_rate_util.CalculateNewPassRate(1.0, 10, 0.5, 10))

  def testGetPassRateNonexistentTest(self):
    swarming_task_output = RunFlakeSwarmingTaskOutput(
        error=None, iterations=0, pass_count=0, started_time=None,
        completed_time=None, has_valid_artifact=True, task_id='task_id')
    self.assertEqual(
  ...
  def testGetPassRateTaskError(self):
    swarming_task_output = RunFlakeSwarmingTaskOutput(
        error=SwarmingTaskError(code=1, message='error'), iterations=None,
        pass_count=None, started_time=None,
        completed_time=None, has_valid_artifact=True, task_id='task_id')
  ...
  def testGetPassRate(self):
    swarming_task_output = RunFlakeSwarmingTaskOutput(
        error=None, iterations=10, pass_count=4, started_time=None,
        completed_time=None, has_valid_artifact=True, task_id='task_id')
    self.assertEqual(
        0.4,
        pass_rate_util.GetPassRate(swarming_task_output))
