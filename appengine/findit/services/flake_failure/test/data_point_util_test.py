...
class DataPointUtilTest(wf_testcase.WaterfallTestCase):
  ...
  def testUpdateAnalysisDataPointsNewDataPointWithError(self):
    ...
    swarming_task_output = RunFlakeSwarmingTaskOutput(
        completed_time=completed_time, error=error,
        has_valid_artifact=has_valid_artifact, iterations=iterations,
        pass_count=pass_count, started_time=started_time, task_id=task_id)
  ...
  def testUpdateAnalysisDataPointsExistingDataPointWithError(self):
    ...
    analysis.data_points = [
        DataPoint.Create(
            commit_position=commit_position, git_hash=revision, pass_rate=0.5,
            iterations=iterations, elapsed_seconds=1800,
  ...
  def testUpdateAnalysisDataPointsExistingDataPointNoError(self):
    ...
    self.assertTrue(data_point.has_valid_artifact)
    self.assertEqual(
        failed_swarming_task_attempts, data_point.failed_swarming_task_attempts)
    self.assertEqual(['task_1', 'task_2'], data_point.task_ids)
