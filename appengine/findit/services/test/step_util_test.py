...
def _MockedListTasks(_http_client, _master_name, _builder_name, build,
                     _step_name):
  if build in [3, 4]:
    return []
  return ['task1']
...
class StepUtilTest(wf_testcase.WaterfallTestCase):
  ...
  @mock.patch.object(swarming, 'ListSwarmingTasksDataByTags', _MockedListTasks)
  ...
  def testGetValidBoundingBuildsForStep(self, *_):
