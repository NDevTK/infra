...
https://findit-for-me.appspot.com/ranked-flakes?bug_key={}.
...
Findit has detected 5+ new flake occurrences of tests below
...
class IssueGeneratorTest(WaterfallTestCase):
  ...
  def testGetSummary(self):
    self.assertEqual('Flakes are found in step.suite.',
  ...
  def testGetFirstCommentWhenBugJustCreated(self):
    ...
    flake_issue.put()
    bug_key_urlsafe = flake_issue.key.urlsafe()
    ...
    wrong_result_link = (
        ...
        'step.suite*&comment=Link%20to%20flake%20details%3A%20https://findit-'
        'for-me.appspot.com/ranked-flakes?bug_key={}').format(bug_key_urlsafe)
    expected_description = _EXPECTED_GROUP_FIRST_COMMENT.format(
        bug_key_urlsafe, wrong_result_link)
    self.assertEqual(expected_description,
                     issue_generator_new.GetFirstCommentWhenBugJustCreated())

  def testGetComment(self):
    issue_generator_old = self._GetIssueGenertor(new_issue=False)
    bug_key_urlsafe = issue_generator_old._flake_issue.key.urlsafe()
    wrong_result_link = (
        'https://bugs.chromium.org/p/chromium/issues/entry?'
        'status=Unconfirmed&labels=Pri-1,Test-Findit-Wrong&'
        'components=Tools%3ETest%3EFindit%3EFlakiness&'
        ...
        'me.appspot.com/ranked-flakes?bug_key={}').format(bug_key_urlsafe)
    expected_description = _EXPECTED_GROUP_COMMENT.format(bug_key_urlsafe,
                                                          wrong_result_link)
    self.assertEqual(expected_description,
                     issue_generator_old.GetComment())

  def testGetFlakyTestCustomizedFieldGroup(self):
    self.assertIsNone(self._GetIssueGenertor().GetFlakyTestCustomizedField())
