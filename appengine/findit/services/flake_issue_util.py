...
def _FlakeHasEnoughOccurrences(unreported_occurrences):
  ...
  CLs for CQ flakes, or at least 3 occurrences for CI flakes.
  ...
  required_ci_flake_occurrences = flake_detection_settings.get(
      'min_required_ci_occurrences_per_day',
      flake_constants.DEFAULT_MINIMUM_REQUIRED_CI_OCCURRENCES_PER_DAY)
  ci_occurrences = [
      occurrence for occurrence in unreported_occurrences
      if occurrence.flake_type == FlakeType.CI_FAILED_STEP
  ]
  if len(ci_occurrences) >= required_ci_flake_occurrences:
    return True
