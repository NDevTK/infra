...
class FlakeGroup(object):
  ...
  def AddFlakeIfBelong(self, flake, occurrences):
    ...
    raise NotImplementedError(
        'AddFlake should be implemented in the child class')
  ...
  def AddFlakeIfBelong(self, flake, occurrences):
    ...
    assert flake_issue == self.flake_issue, (
       ...
       'links to another issue {a_issue}'.format(
           flake=flake.key.urlsafe(), issue=self.flake_issue.issue_id,
           a_issue=flake_issue.issue_id if flake_issue else None))
...
def GetFlakeGroupsForActionsOnBugs(flake_tuples_to_report):
  ...
  Cases:
    ...
    2. Flake3 does't link to any FlakeIssue. It has the same
    ...
    3. Flake4 does't link to any FlakeIssue. It failed in the same builds as
