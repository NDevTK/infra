...
def _FetchTasks():
  ...
  query = swarming_task_queue_request.SwarmingTaskQueueRequest.query(
      swarming_task_queue_request.SwarmingTaskQueueRequest.state ==
...
def EnqueueTask(request):
  ...
  request.state = (swarming_task_queue_request.SwarmingTaskQueueState.SCHEDULED)
  request.request_time = time_util.GetUTCNow()
  request.put()


def GetTaskRequest(task_id):
  query = swarming_task_queue_request.SwarmingTaskQueueRequest.query(
      swarming_task_queue_request.SwarmingTaskQueueRequest.swarming_task_id ==
      task_id)
  more = True
  cursor = None
  while more:
    results, cursor, more = query.fetch_page(1, start_cursor=cursor)
    if results:
      return results[0]
...
def CompleteTask(task_id):
  ...
  analysis = ndb.Key(urlsafe=taskqueue_task.analysis_urlsafe_key).get()
  assert analysis

  # TODO(crbug.com/780543): Add all needed fields to BaseSwarmingTask and use
  #   it here.
  ...
  task.created_time = time_util.DatetimeFromString(
      result_data.get('created_ts'))
  task.started_time = time_util.DatetimeFromString(
      result_data.get('started_ts'))
  task.completed_time = time_util.DatetimeFromString(
