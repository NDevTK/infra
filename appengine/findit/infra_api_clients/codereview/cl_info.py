...
class ClInfo(object):  # pragma: no cover
  ...
  def GetCulpritPatchsetIdByRevision(self, revision):
    for commit in self.commits:
      if commit.revision == revision:  # pragma: no branch
        return commit.patchset_id
    return None  # pragma: no cover

  def GetRevertCLByRevision(self, revision):
    culprit_patchset_id = self.GetCulpritPatchsetIdByRevision(revision)
