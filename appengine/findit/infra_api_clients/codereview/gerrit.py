...
class Gerrit(codereview.CodeReview):  # pragma: no cover
  ...
  def _Get(self, *parts, **kwargs):
    ...
    parts = ['a'] + list(parts)
    url = 'https://%s/%s' % (self._server_hostname, '/'.join(parts))
    query_string_params = kwargs.get('params')
    ...
    status_code, content = self.HTTP_CLIENT.Get(
      url, params=query_string_params, headers=headers)
    return self._HandleResponse(status_code, content)

  def _Post(self, *parts, **kwargs):
    parts = ['a'] + list(parts)
    url = 'https://%s/%s' % (self._server_hostname, '/'.join(parts))
  ...
  def PostMessage(self, change_id, message):
    ...
    return result is not None  # A successful post will return a empty list.
  ...
  def _ParseClInfo(self, change_info, change_id):
    ...
    # If the status is merged, look at the commit details for the current commit
    if result.closed:
      ...
      if committer !=  COMMIT_BOT_EMAIL:
        result.AddCqAttempt(patchset_id, committer, commit_timestamp)

    # TO FIND COMMIT ATTEMPTS
    # In messages look for first-lines like "Patch Set 1: Commit-Queue+2"
