...
def _RevertCulprit(repo_name, revision):
  ...
  reverts = culprit_cl_info.GetRevertCLByRevision(revision)
  ...
  if not revert_change_id and culprit.revert_status != status.RUNNING:
    _UpdateCulprit(repo_name, revision, revert_status=status.RUNNING)
    ...
    revert_change_id = codereview.CreateRevert(
      ...
      culprit_cl_info.GetCulpritPatchsetIdByRevision(revision))
  ...
  if not revert_change_id:  # pragma: no cover
    logging.info('Another analysis is reverting %s/%s.' % (repo_name, revision))
    return CREATED_BY_FINDIT
