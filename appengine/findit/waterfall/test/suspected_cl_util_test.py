...
class SuspectedCLUtilTest(wf_testcase.WaterfallTestCase):
  ...
  def testGetCulpritChangeLog(self, mock_fn):
    ...
    change_log = suspected_cl_util.GetCulpritChangeLog(
        'chromium', 'rev')
    self.assertEqual(change_log.commit_position, 123)
    self.assertEqual(change_log.code_review_url, 'code_review_url')
