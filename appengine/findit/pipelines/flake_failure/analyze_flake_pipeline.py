  ...
  def RunImpl(self, parameters):
    ...
    if commit_position_to_analyze is None:
      ...
      if culprit_commit_position is None:
        ...
        yield FinishAnalyzeFlakePipeline(
            self.CreateInputObjectInstance(
...
class FinishAnalyzeFlakeInput(StructuredObject):
  # The urlsafe key to the MasterFlakeAnalysis in progress.
  analysis_urlsafe_key = basestring


class FinishAnalyzeFlakePipeline(GeneratorPipeline):
  """Pipeline for setting an analysis' final status when all other pipelines"""
  input_type = FinishAnalyzeFlakeInput
  ...
  def RunImpl(self, parameters):
    ...
    status = (
        analysis_status.ERROR if analysis.error else analysis_status.COMPLETED)
    analysis.Update(end_time=time_util.GetUTCNow(), status=status)

    # TODO(crbug.com/847644): Check for analysis.error and report to ts_mon.
