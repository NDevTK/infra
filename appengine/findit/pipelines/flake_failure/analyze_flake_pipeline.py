...
class AnalyzeFlakePipeline(GeneratorPipeline):
  ...
  def OnFinalized(self, parameters):
    if self.IsRootPipeline():
      ...
      # Error may or may not be set depending if detected.
      error = analysis.error

      if self.was_aborted:
        # Aborted analyses should always have an error associated.
      ...
      analysis.Update(
          error=error, end_time=time_util.GetUTCNow(), status=status)
