...
class AnalyzeFlakePipelineTest(WaterfallTestCase):
  ...
  def testOnAbortRootPipeline(self, mocked_error):
    analysis = MasterFlakeAnalysis.Create('m', 'b', 123, 's', 't')
    analysis.Save()
  ...
  @mock.patch.object(AnalyzeFlakePipeline, 'IsRootPipeline', return_value=False)
  @mock.patch.object(flake_analysis_util,
                     'ReportPotentialErrorToCompleteAnalysis')
  def testOnAbortRecursiveCall(self, mocked_error, _):
    analysis = MasterFlakeAnalysis.Create('m', 'b', 123, 's', 't')
    analysis.Save()
