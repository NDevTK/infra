...
class CollectSwarmingTaskResultsPipelineTest(wf_testcase.WaterfallTestCase):
  ...
  @mock.patch.object(test_swarming, 'GetsStepsToCollectSwarmingTaskResults')
  ...
  @mock.patch.object(
      test_swarming, 'GetsStepsToCollectSwarmingTaskResults', return_value=[])
  def testRunImplNoSteps(self, mock_fn, mock_save, mocked_schedule, _):
    p = CollectSwarmingTaskResultsPipeline(self.pipeline_input)
    p.RunImpl(self.pipeline_input)
  ...
  @mock.patch.object(
      test_swarming,
      'GetsStepsToCollectSwarmingTaskResults',
      return_value=['step'])
  ...
  @mock.patch.object(CollectSwarmingTaskResultsPipeline, 'ScheduleCallbackTask')
  @mock.patch.object(test_swarming, 'CollectSwarmingTaskResults')
  ...
  @mock.patch.object(CollectSwarmingTaskResultsPipeline, 'ScheduleCallbackTask')
  @mock.patch.object(
      test_swarming, 'CollectSwarmingTaskResults', return_value=None)
  def testCallbackImplRunning(self, mock_fn, mock_schedule, mock_save):
    ...
    mock_fn.assert_called_once_with(self.pipeline_input, ['step'])
    mock_schedule.assert_called_once_with(countdown=240)
    mock_save.assert_called_once_with({'steps': ['step'], 'callback_count': 4})

  @mock.patch.object(CollectSwarmingTaskResultsPipeline, 'ScheduleCallbackTask')
  @mock.patch.object(
      test_swarming,
      'CollectSwarmingTaskResults',
