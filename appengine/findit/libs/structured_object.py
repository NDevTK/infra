...
A subclass should define "public" attributes in the class instead of within the
constructor, and those attributes will be serialized to a dict or deserialized
from a dict. The constructor in the subclass should accept keyword parameters
only, it must absorb those that are not defined attributes and pass the
remaining ones to the base class StructuredObject.
...
class StructuredObject(object):
  ...
  def ToDict(self):
    ...
    for name, value_type in defined_attributes.iteritems():
      assert name in self._data, '%s.%s is not set' % (
          self.__class__.__name__, name)
      value = self._data[name]
      if (value is not None and
          issubclass(value_type, StructuredObject) and
  ...
  def FromDict(cls, data):
    ...
    assert len(undefined_attributes) == 0, (
        '%s not defined in %s' % (','.join(undefined_attributes), cls.__name__))
    instance = cls()
    for name, value_type in defined_attributes.iteritems():
      if name not in data:
