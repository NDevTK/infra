...
from a dict. The constructor in the subclass should accept key-value parameters
only, it must absorb those that are not defined attributes and pass the
remaining ones to the base class SerializableObject.
...
class SerializableObject(object):
  ...
  def __getattribute__(self, name):
    ...
    # function, or method with an class instance, e.g. self.__class__ or
  ...
  def ToDict(self):
    ...
    for name, value in self._data.iteritems():
      if (issubclass(defined_attributes[name], SerializableObject) and (
          not self._type_validation_func or
          not self._type_validation_func(name, value))):
        value = value.ToDict()
      data[name] = value
    return data
