...
class BaseTestResults(object):
  ...
  def RemoveAllPrefixesFromTestName(test):
    ...
    This method is only applicable to certain test results format. For those
    don't need this method, return test name directly.

    Args:
      test (str): A test's name.

    Returns:
      base_test (str): A test's name after stripping prefix.
    """
    return test
