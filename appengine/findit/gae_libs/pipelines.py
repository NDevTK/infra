...
* SyncPipeline: A base pipeline class for synchronous pipelines that return a
  value immediately after some computation.

  class Info(StructuredObject):
    threshold = int
    l = list

  class FilterSum(SyncPipeline):
    input_type = Info
    output_type = int
...
* GenPipeline: A base pipeline class for generator piplines that spawns other
  sub-pipelines for further computation.
  ...
  class ExtractFailureSignals(SyncPipeline):
  ...
  class IdentifyRegressionRange(SyncPipeline):
    input_type = BuildInfo
  ...
  class IdentifyCulprit(SyncPipeline):
  ...
  class AnalyzeBuild(GenPipeline):
    input_type = BuildInfo
    def RunImpl(self, info):
...
* AsyncPipeline: A base pipeline class for asynchronous pipelines that depend on
  external data/actions, and wait for callback.
  ...
  class RerunTests(AsyncPipeline):
    input_type = TestInfo
...
def _ValidateType(parameter_type, parameter_name):
  ...
  assert any(issubclass(parameter_type, t) for t in _SUPPORTED_TYPES), (
      '%s must be in supported types: %r.' % (
          parameter_name, _SUPPORTED_TYPE_NAMES))
