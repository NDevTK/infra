...
def _ValidateType(given_type, name):
  """Asserts the given type is supported."""
  assert type(given_type) == types.TypeType, (
      '%s must be defined with a class or a type' % name)
  assert any(issubclass(given_type, t) for t in _SUPPORTED_TYPES), (
      '%s must be in supported types: %r.' % (name, _SUPPORTED_TYPE_NAMES))


def _ConvertInputObjectToPipelineParameters(input_type, args, kwargs):
  """Coverts the serializable object into arguments to pass over to pipeline."""
  # In creation of the pipeline, serializable input should be encoded so that
  ...
  assert len(kwargs) == 0, 'Pipeline expects no key-value parameters!'

  if isinstance(args[0], pipeline.PipelineFuture):
    # Accept Future output of another pipeline.
    return args, kwargs

  assert isinstance(args[0], input_type), (
      'Expected input of type %s, but got %s' % (
          input_type.__name__, type(args[0]).__name__))
  if not issubclass(input_type, SerializableObject):
    return args, kwargs
...
def _ConvertPipelineParametersToInputObject(input_type, args, kwargs):
  """Converts the given pipeline parameters back to a serializable object."""
  assert len(args) == 1, ('Expected 1 positional argument, but got %d' %
                          len(args))
  if isinstance(args[0], basestring) and args[0] == _ENCODED_PARAMETER_FLAG:
...
def _ConvertToPipelineOutput(output):
  ...
  if isinstance(output, SerializableObject):
...
class BasePipeline(pipeline.Pipeline):
  ...
  def root_pipeline_status_path(self):
    ...
    return '%s/status?root=%s&auto=false' % (
        self.base_path, self.root_pipeline_id)
  ...
  def finalized(self):
    ...
    arg = _ConvertPipelineParametersToInputObject(
        self.input_type, self.args, self.kwargs)
    if self.was_aborted:
      self.OnAbort(arg)
...
class GenPipeline(BasePipeline):
  ...
  def CreateInstance(self, cls, **kwargs):
    """Returns serializable instance that accepts pipeline Future as value.

    Args:
      cls (class): A subclass of SerializableObject.
...
class AsyncPipeline(BasePipeline):
  ...
  Callback should use function `Complete` to report the pipeline output.
  """
  async = True
