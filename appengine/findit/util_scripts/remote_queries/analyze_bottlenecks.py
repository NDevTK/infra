...
def _AggregateForCustomTransition(options, sorted_records, loaded_records,
                                  from_regex, to_regex):
...
def _AggregateForTryJobLatency(options, sorted_records, loaded_records):
  ...
  return _AggregateForCustomTransition(options, sorted_records, loaded_records,
                                       from_regex, to_regex)

def _AggregateForSwarmingLatency(options, sorted_records, loaded_records):
  options.coalesce_middle = True
  options.paired_only = False
  options.min_transition_count = 1
  options.show_composites = True
...
def _AggregateTransitions(options, sorted_records, loaded_records):
  if options.swarming_latency:
    return _AggregateForSwarmingLatency(options, sorted_records, loaded_records)
  elif options.tryjob_latency:
    return _AggregateForTryJobLatency(options, sorted_records, loaded_records)
  coalesce = _Coalesce(options)
  transitions = defaultdict(list)
  adjacent = defaultdict(int)
  examples = {}
