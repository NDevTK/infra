...
def GetTestFailures(test_failure_entities):
  ...
  for test_failure_entity in test_failure_entities:
    print test_failures[test_failure_entity.step_ui_name]
...
class TestFailure(AtomicFailure):
  ...
  def _IsTheSameFailure(self, step_ui_name, failure_identifier):
    if self.step_ui_name != step_ui_name:
      return False

    if not failure_identifier and not self.GetFailureIdentifier():
      # No test failure info, still considers the failure is the same since
      # step name is the same.
      return True

    assert isinstance(failure_identifier, basestring), (
        'TestFailure {} requires string identifier, got {}.'.format(
            self.key.urlsafe(), failure_identifier))
    return self.GetFailureIdentifier() == failure_identifier
...
class TestRerunBuild(LuciBuild):
  ...
  def GetFailuresInBuild(self):
    ...
    for failure in self.failures or []:
      if not failure.test:
        test_failures[failure.step_ui_name] = test_failures[failure
                                                            .step_ui_name]
      else:
