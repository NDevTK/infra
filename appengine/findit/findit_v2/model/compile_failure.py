...
class CompileFailure(AtomicFailure):
  ...
  def _IsTheSameFailure(self, step_ui_name, failure_identifier):
    ...
    if not failure_identifier and not self.GetFailureIdentifier():
      # No target failure info, still considers the failure is the same since
      # step name is the same.
      return True

    assert isinstance(failure_identifier, frozenset), (
        'CompileFailure {} requires frozenset identifier, got {}.'.format(
