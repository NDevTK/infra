...
def AllLaterBuildsHaveOverlappingFailure(context, build):
  ...
  them fail with some overlap with the original failure (based on failed step
  names).
  ...
  Returns:
    True if all completed builds on the builder after the orignal failure are
  ...
  latest_builds = GetRecentCompletedBuilds(builder_id)
  original_failed_steps = _StepNamesOnly(GetFailedStepsInBuild(context, build))
  for newer_build in latest_builds:
    if newer_build.number <= build.number:
      break
    if newer_build.status == common_pb2.SUCCESS:
      logging.info('Found later build that succeeded')
      return False
    ...
    if (not original_failed_steps & new_failed_steps and
      ...
      logging.info(
          'All steps failed in original build %d succeeded in later build %d',
          build.number, newer_build.number)
      return False
  return True
...
def GetRecentCompletedBuilds(builder_id, page_size=20, at_or_after_build=0):
  ...
  If given, filter out builds with build number less than <at_or_after_build>.
  ...
  Args:
    ...
    at_or_after_build (int): If greater than zero, exclude all build with a
        number lower than this.
  """
  ...
  if search_builds_response:
    return sorted([
        ...
        if build.number >= at_or_after_build
    ],
