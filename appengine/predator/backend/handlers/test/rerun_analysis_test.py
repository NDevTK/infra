...
from backend.handlers.rerun_analysis import IterateCrashBatches
...
class RerunAnalysisTest(AppengineTestCase):
  ...
  def setUp(self):
    super(RerunAnalysisTest, self).setUp()
  ...
  def testIterateCrashBatches(self, _, mock_reinitialize):
    ...
    for _ in IterateCrashBatches(
      ...
      count += 1
      self.assertEqual(mock_reinitialize.call_count, count)
  ...
  @mock.patch('backend.handlers.rerun_analysis.IterateCrashBatches')
  @mock.patch('common.crash_pipeline.RerunPipeline.start')
  def testHandleGetWhenThereIsOneCrashAnalyses(self, mock_run, mock_iterate):
    ...
    self.mock_current_user(user_email='test@chromium.org', is_admin=True)
    response = self.test_app.get(
        '/process/rerun-analysis?start_date=2017-06-01&end_date=2017-06-02')
    self.assertEqual(response.status_int, 200)
  ...
  def testHandleGetWhenThereAreMultipleCrashAnalyses(
    ...
    self.mock_current_user(user_email='test@chromium.org', is_admin=True)
    response = self.test_app.get(
        '/process/rerun-analysis?start_date=2017-06-01&end_date=2017-06-03')
    self.assertEqual(response.status_int, 200)
