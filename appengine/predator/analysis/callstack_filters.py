...
class FilterFramesBeforeAndInBetweenSignatureParts(CallStackFilter):
  ...
  def __call__(self, stack_buffer):
    ...
    def _MatchSignatureWithFrames(frames, signature_parts):
      for frame in frames:
    ...
    def _FilterFramesBeforeAndInBetweenSignatureParts(frames, signature_parts):
      frame_index = 0
      signature_index = 0
      filtered_index = []
    ...
    match, valid_signature_parts = _MatchSignatureWithFrames(
        stack_buffer.frames, self.signature_parts)
    if match:
      # Filter all the stack frames before signature.
      stack_buffer.frames = _FilterFramesBeforeAndInBetweenSignatureParts(
