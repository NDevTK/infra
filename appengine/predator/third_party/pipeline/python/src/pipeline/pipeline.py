...
class Pipeline(with_metaclass(_PipelineMeta, object)):
  ...
  def fill(self, name_or_slot, value):
    ...
    """
    if isinstance(name_or_slot, six.string_types):
...
class _PipelineContext(object):
  ...
  def notify_barriers(self,
    ...
    pipelines_to_trigger = dict(
        list(zip(pipeline_keys_to_trigger, db.get(pipeline_keys_to_trigger))))
    task_list = []
    updated_barriers = []
    for barrier in barriers_to_trigger:
...
class _FanoutAbortHandler(object):
...
def _get_internal_status(pipeline_key=None,
  ...
  for value_dict in itertools.chain(output['args'],
                                    iter(output['kwargs'].values())):
