...
# later.
class UMA_RegressionAnalysis(CrashAnalysis):
  """Represents an analysis of a UMA Sampling Profiler Regression."""

  def Reset(self):
    super(UMA_RegressionAnalysis, self).Reset()

  def Initialize(self, regression_data):
    """(Re)Initializes a CrashAnalysis ndb.Model from ``UMA_RegressionData``."""
    super(UMA_RegressionAnalysis, self).Initialize(regression_data)
