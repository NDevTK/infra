...
# later.
class UMA_SamplingProfilerAnalysis(CrashAnalysis):
  """Represents an analysis of a UMA Sampling Profiler Regression."""

  def Reset(self):
    super(UMA_SamplingProfilerAnalysis, self).Reset()

  def Initialize(self, regression_data):
    """(Re)Initializes a CrashAnalysis ndb.Model from ``regression_data``."""
    super(UMA_SamplingProfilerAnalysis, self).Initialize(regression_data)
