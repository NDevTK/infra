...
def _process_pull_task_batch(queue_name, dataset):
  ...
  build_annotation_keys = [
      ndb.Key(
          model.BuildAnnotations, model.BuildAnnotations.ENTITY_ID, parent=k)
      for k in build_keys
  ]
  ...
  # Convert fetched builds and build annotations to v2 format.
  v2_builds = []
...
def _build_to_v2(bid, build, build_ann):
  ...
  if build_ann:
    ...
    ann_step.ParseFromString(build_ann.annotations_binary)
