...
def enqueue_bq_export_async(build):  # pragma: no cover
  ...
  return enqueue_pull_task_async(
      _queue_name(build.experimental),
...
class CronExportBuilds(webapp2.RequestHandler):  # pragma: no cover
  ...
  def get(self):
    _process_pull_task_batch(
        _queue_name(self.expereimental),
        'builds_experimental' if self.experimental else 'builds')
...
def _queue_name(expereimental):
  return 'bq-export-experimental' if expereimental else 'bq-export'
