...
class CronExportBuilds(webapp2.RequestHandler):  # pragma: no cover
  ...
  def get(self):
    _process_pull_task_batch(False)
    _process_pull_task_batch(True)
...
def _process_pull_task_batch(experimental):
  ...
  q = taskqueue.Queue(_QUEUE_NAME)
  tasks = q.lease_tasks_by_tag(
      lease_duration.total_seconds(), 1000, tag=_task_tag(experimental))
  if not tasks:
    return
  ...
  if to_export_builds:
    ...
    tasks = [t for t in tasks if t not in failed_tasks]
