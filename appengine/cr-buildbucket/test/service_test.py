...
class BuildBucketServiceTest(testing.AppengineTestCase):
  ...
  def mock_cannot(self, action, bucket=None):
    def can_async(requested_bucket, requested_action, _identity=None):
      print 'can_async(%r, %r, %r)' % (requested_bucket, requested_action, _identity)
      match = (
  ...
  def add(self, bucket, **request_fields):
    return service.add(service.add_request(bucket, **request_fields))
  ...
  def test_add_builder_tag_none(self):
    self.assertEqual(service._add_builder_tag(None, {}), [])

  def test_add_builder_tag_no_params(self):
    self.assertEqual(service._add_builder_tag([], None), [])

  def test_add_builder_tag_unspecified(self):
    self.assertEqual(service._add_builder_tag([], {'foo': 'bar'}), [])

  def test_add_builder_tag_multi(self):
    tags = ['builder:foo', 'builder:foo']
    self.assertEqual(service._add_builder_tag(tags, {'foo': 'bar'}), tags)

  def test_add_builder_tag_different(self):
    tags = ['builder:foo', 'builder:bar']
    with self.assertRaises(errors.InvalidInputError):
        service._add_builder_tag(tags, {'foo': 'bar'})

  def test_add_builder_tag_coincide(self):
    tags = ['builder:foo']
    self.assertEqual(service._add_builder_tag(tags, {'builder_name': 'foo'}),
                     tags)

  def test_add_builder_tag_conflict(self):
    tags = ['builder:foo']
    with self.assertRaises(errors.InvalidInputError):
      service._add_builder_tag(tags, {'builder_name': 'bar'})
  ...
  def test_add_many(self):
    ...
    results = service.add_many_async([
      service.add_request(bucket='chromium', tags=['buildset:a']),
      service.add_request(bucket='chromium', tags=['buildset:a']),
    ]).get_result()
    ...
    self.assertEqual(len(index.entries), 2)
    self.assertEqual(index.entries[1].build_id, results[1][0].key.id())
    self.assertEqual(index.entries[1].bucket, results[1][0].bucket)

  def test_add_many_invalid_input(self):
    results = service.add_many_async([
      service.add_request(bucket='chromium', tags=['buildset:a']),
      service.add_request(bucket='chromium', tags=['buildset:a', 'x']),
    ]).get_result()
    self.assertIsNotNone(results[0][0])
    self.assertIsNone(results[0][1])
  ...
  def test_add_many_auth_error(self):
    ...
    results = service.add_many_async([
      service.add_request(bucket='chromium', tags=['buildset:a']),
      service.add_request(bucket='forbidden', tags=['buildset:a']),
    ]).get_result()
    self.assertIsNotNone(results[0][0])
    self.assertIsNone(results[0][1])
    self.assertIsNone(results[1][0])
    ...
    self.assertIsInstance(results[1][1], auth.AuthorizationError)
