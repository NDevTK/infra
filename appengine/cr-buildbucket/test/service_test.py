...
class BuildBucketServiceTest(testing.AppengineTestCase):
  ...
  def test_add_many_one_fails(self):
    # Mock model.Build.put_async so that it fails for one of requests.
    orig_put_async = model.Build.put_async
    datastore_flaked = Exception('datastore flaked')
    def put_async(self):
      if 'unlucky' in self.bucket:
        raise datastore_flaked
      return orig_put_async(self)
    self.mock(model.Build, 'put_async', put_async)

    results = service.add_many_async([
      service.BuildRequest(bucket='chromium', tags=['buildset:a']),
      service.BuildRequest(bucket='unlucky_bucket', tags=['buildset:a'])
    ]).get_result()
    self.assertEqual(len(results), 2)
    self.assertIsInstance(results[0][0], model.Build)
    self.assertIsNone(results[0][1])
    self.assertIsNone(results[1][0])
    self.assertIs(results[1][1], datastore_flaked)
