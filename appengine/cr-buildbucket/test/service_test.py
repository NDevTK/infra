...
class BuildBucketServiceTest(testing.AppengineTestCase):
  ...
  def add(self, bucket, **request_fields):
    return service.add(service.BuildRequest(self.chromium_project_id, bucket, **request_fields))
  ...
  def test_add_without_project(self):
    with self.assertRaises(auth.AuthorizationError):
      service.add(service.BuildRequest(None, 'chromium'))
  ...
  def test_add_many(self):
    ...
    results = service.add_many_async([
      service.BuildRequest(project=self.chromium_project_id, bucket='chromium', tags=['buildset:a']),
      service.BuildRequest(project=self.chromium_project_id, bucket='chromium', tags=['buildset:a']),
    ]).get_result()
  ...
  def test_add_many_invalid_input(self):
    results = service.add_many_async([
      service.BuildRequest(project=self.chromium_project_id, bucket='chromium', tags=['buildset:a']),
      service.BuildRequest(project=self.chromium_project_id, bucket='chromium', tags=['buildset:a', 'x']),
    ]).get_result()
    self.assertEqual(len(results), 2)
    self.assertIsNotNone(results[0][0])
    self.assertIsNone(results[0][1])
  ...
  def test_add_many_auth_error(self):
    ...
    with self.assertRaises(auth.AuthorizationError):
      service.add_many_async([
        service.BuildRequest(project=self.chromium_project_id, bucket='chromium', tags=['buildset:a']),
        service.BuildRequest(project='forbidden', bucket='forbidden', tags=['buildset:a']),
      ]).get_result()
