...
def add_many_async(build_request_list):
  ...
  # 6. Put new builds.
  # 7. Cache new builds.
  # and while doing so we group similar async operations together.
  ...
  buckets = set(r.bucket for r in build_request_list if r)
  ...
  # Phase 6 and 7: for each pending request, put new build and
  # for those requests with client operation id, cache the new builds.
  put_builds = {
  ...
  for i, put in put_builds.iteritems():
    build = put.get_result()
    assert build
    results[i] = (build, None)
    if build_request_list[i].client_operation_id:
      save_cache_futures.append(ctx.memcache_set(
          build_request_list[i]._client_op_memcache_key(),
          build.key.id(),
          60))
  if save_cache_futures:
    yield save_cache_futures
