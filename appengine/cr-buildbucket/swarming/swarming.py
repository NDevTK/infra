...
def _create_task_def_async(
  ...
  if builder_cfg.expiration_secs > 0:
    # TODO(maruel): Should be subtracted from previous task slices.
  ...
  for t in task['task_slices']:
    ...
    # It doesn't matter that cache_fallbacks gets overridden within the loop,
    # since cache fallback only works when there's exactly one task slice.
