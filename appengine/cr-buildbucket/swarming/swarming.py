...
def _create_task_def_async(
  ...
  for t in task['task_slices']:
    ...
    if c and not fallback_allowed:
      raise errors.InvalidInputError(
          'Multiple task_slices cannot be used in combination with a builder '
          'using CacheEntry.wait_for_warm_cache_secs'
      )
    cache_name = c
    fallback_secs = f
  ...
  if cache_name:
    ...
    # Guard against the case where the expiration is low compared to the
    # fallback.
    exp = int(task['task_slices'][1]['expiration_secs'])
    exp = max(fallback_secs, exp - fallback_secs)
    task['task_slices'][1]['expiration_secs'] = str(exp)
...
def _add_named_caches(builder_cfg, task_properties):
  ...
  for c in builder_cfg.caches:
    ...
    if c.wait_for_warm_cache_secs:
      # swarmingcfg.validate_builder_cfg() validates that at most one
      # CacheEntry.wait_for_warm_cache_secs can be used.
      cache_name = c.name
