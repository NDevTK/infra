...
with 1m delay, and continuously synchronizes states.
...
def _compute_env_prefixes(build):
  ...
  # TODO(iannucci): it is possible that the order of env variable declarations
  # is significant, but the user does not have any control over it.
  return [{
      'key': key,
      'value': value,
...
def _kitchen_command(build, settings):
  ...
  ret = [
      ...
      'tmp',
      '-properties',
      api_common.properties_to_json(_compute_properties(build)),
...
def _compute_properties(build):
  ...
  if 'recipe' not in props.fields:  # pragma: no cover
    return props
