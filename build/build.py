...
def build_pkg(cipd_exe, pkg_def, out_file, package_vars, sign_id=None):
  ...
  Returns:
    ({'package': <name>, 'instance_id': <hash>}, [on_change_tag, ...])
  ...
  try:
    ...
    if 'mac_bundle' in pkg_def.pkg_def:
      ...
      if 'codesign' in bundle_def:
        ...
        if sign_id:
          ...
          cmd.extend(['--sign', '-'])
        cmd.append(os.path.join(build_root, bundle_def['name']))
    ...
    print '%s %s %s' % (info['package'], info['instance_id'], on_change_tags)
    return info, on_change_tags
  finally:
